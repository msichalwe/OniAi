...
import type { OniAIConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredOniAITmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "oni.log"); // legacy single-file path

const LOG_PREFIX = "oni";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OniAIConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => OniAIConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ONI_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[oni] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
