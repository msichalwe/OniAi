...
`;
    ...
    expect(parsed.oni.emoji).toBe("💾");
    expect(parsed.oni.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.oni.emoji).toBe("📝");
    expect(parsed.oni.events).toEqual(["command"]);
    expect(parsed.oni.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.oni.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"oni": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"oni": {"events": ["test"]}}');
  });
...
describe("resolveOniAIMetadata", () => {
  it("extracts oni metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOniAIMetadata(frontmatter);
  ...
  it("returns undefined when oni key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        oni: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OniAI" },
            { id: "npm", kind: "npm", package: "@oni/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@oni/hook");
  });
    ...
    "oni":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OniAI" }],
      },
  }
---
...
`;
    ...
    const oni = resolveOniAIMetadata(frontmatter);
    expect(oni).toBeDefined();
    expect(oni?.emoji).toBe("💾");
    expect(oni?.events).toEqual(["command:new", "command:reset"]);
    expect(oni?.requires?.config).toEqual(["workspace.dir"]);
    expect(oni?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const oni = resolveOniAIMetadata(frontmatter);
    expect(oni?.emoji).toBe("disk");
    expect(oni?.events).toEqual(["command:new"]);
  });
});
