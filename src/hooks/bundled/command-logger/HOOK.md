...
homepage: https://docs.oni.ai/automation/hooks#command-logger
metadata:
  {
    "oni":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.oni/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.oni/logs/commands.log ~/.oni/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/oni`:
   ```
   /home/username/.oni/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
