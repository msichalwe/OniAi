...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => OniAIConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("oni-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): OniAIConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
