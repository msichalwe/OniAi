...
This directory contains hooks that ship with OniAI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.oni/oni.json`:
...
2. Restart gateway: `pkill -9 -f 'oni.*gateway' && pnpm oni gateway`
3. Enable the hook: `oni hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.oni.ai/automation/hooks
