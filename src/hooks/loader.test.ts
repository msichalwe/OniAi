...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ONI_BUNDLED_HOOKS_DIR"]);
    process.env.ONI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
