...
describe("canvas host state dir defaults", () => {
  it("uses ONI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("oni-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OniAI Canvas");
      } finally {
        await handler.close();
      }
    });
