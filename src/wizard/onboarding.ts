...
export async function runOnboardingWizard(
  ...
  let baseConfig: OniAIConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("oni doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("oni configure")}.`;
  ...
  let nextConfig: OniAIConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
