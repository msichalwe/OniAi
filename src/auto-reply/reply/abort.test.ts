...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oni-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop oni!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@oni_bot", { botUsername: "oni_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@oni_bot", { botUsername: "oni_bot" })).toBe(true);
