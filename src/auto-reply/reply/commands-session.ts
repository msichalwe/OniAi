...
import { scheduleGatewaySigusr1Restart, triggerOniAIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting OniAI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOniAIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting OniAI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
