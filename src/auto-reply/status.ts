...
type AgentDefaults = NonNullable<NonNullable<OniAIConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.oni/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
const formatVoiceModeLine = (config?: OniAIConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🤖 OniAI ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: OniAIConfig): string {
  const lines = ["ℹ️ Help", ""];
