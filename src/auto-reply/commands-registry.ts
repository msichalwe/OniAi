...
export function isCommandEnabled(cfg: OniAIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OniAIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OniAIConfig),
...
export function getCommandDetection(_cfg?: OniAIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OniAIConfig) {
