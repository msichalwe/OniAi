  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.oni/logs/oni.log" },
        hooks: {
          path: "~/.oni/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".oni", "logs", "oni.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".oni", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".oni", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".oni", "agents", "main"));
