...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ONI_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-oni",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-oni#general");
  });
  ...
  it("derives session transcripts dir from ONI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ONI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.oni");
  ...
  it("resolves cross-agent paths when ONI_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.oni"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
