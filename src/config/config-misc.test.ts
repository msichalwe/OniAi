...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OniAISchema.safeParse({
      $schema: "https://oni.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://oni.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OniAISchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OniAISchema.safeParse({
      cron: {
        enabled: true,
