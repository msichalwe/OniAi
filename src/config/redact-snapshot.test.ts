...
import type { ConfigFileSnapshot } from "./types.oni.js";
import { OniAISchema } from "./zod-schema.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.oni/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/oni/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/oni/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/oni/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/oni/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(OniAISchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("restoreRedactedValues", () => {
  ...
  it("preserves non-sensitive fields unchanged", () => {
    ...
    const original = {
      ui: { seamColor: "#0088cc" },
      gateway: { port: 19100, auth: { token: "real-secret" } },
    };
    const result = restoreRedactedValues(incoming, original) as typeof incoming;
  ...
  it("round-trips config through redact → restore", () => {
    const originalConfig = {
      gateway: { auth: { token: "gateway-auth-secret-token-value" }, port: 19100 },
      channels: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OniAISchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OniAIConfig";
    const hints = mapSensitivePaths(OniAISchema, "", {});
