...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("oni-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "oni-sandbox-browser-global" },
      agentBrowser: { network: "oni-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("oni-sandbox-browser-agent");
  });
