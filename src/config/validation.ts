...
import type { OniAIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OniAIConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OniAIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OniAISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OniAIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OniAIConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OniAIConfig,
  };
}
