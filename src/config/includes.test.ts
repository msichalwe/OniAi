...
const ETC_ONI_DIR = path.join(ROOT_DIR, "etc", "oni");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "oni.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOniAIPath(...parts: string[]) {
  return path.join(ETC_ONI_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcOniAIPath("agents.json");
      ...
      },
      {
        ...
        files: {
          [configPath("gateway.json")]: {
            gateway: { port: 19100, bind: "loopback" },
          },
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-includes-symlink-"));
      try {
