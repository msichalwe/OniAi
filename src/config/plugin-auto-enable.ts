...
function isWhatsAppConfigured(cfg: OniAIConfig): boolean {
...
function isGenericChannelConfigured(cfg: OniAIConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OniAIConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OniAIConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OniAIConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: OniAIConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OniAIConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OniAIConfig, pluginId: string): OniAIConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
