...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".oni");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ONI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ONI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ONI_STATE_DIR.
