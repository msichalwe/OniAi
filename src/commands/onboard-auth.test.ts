...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ONI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("oni-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-oauth-sync-"));
    ...
    process.env.ONI_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-oauth-nosync-"));
    process.env.ONI_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ONI_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-oauth-external-"));
    process.env.ONI_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ONI_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ONI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("oni-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
