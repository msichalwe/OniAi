...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OniAI status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`oni devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("oni devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("oni devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("oni security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("oni security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.oni.ai/faq");
  runtime.log("Troubleshooting: https://docs.oni.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("oni status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("oni logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("oni status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("oni gateway probe")}`);
  }
}
