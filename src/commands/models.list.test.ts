...
const ensureOniAIModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/oni-agent/auth.json" });
const resolveOniAIAgentDir = vi.fn().mockReturnValue("/tmp/oni-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/oni-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureOniAIModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/oni-agent");
  });
