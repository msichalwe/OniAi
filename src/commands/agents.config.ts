...
function resolveAgentName(cfg: OniAIConfig, agentId: string) {
...
function resolveAgentModel(cfg: OniAIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OniAIConfig): AgentSummary[] {
