...
export const resolveOniAIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
export const resolveGatewayProgramArguments = vi.fn().mockResolvedValue({
  programArguments: ["node", "cli", "gateway", "--port", "19100"],
}) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOniAIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOniAIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ONI_STATE_DIR;
  originalUpdateInProgress = process.env.ONI_UPDATE_IN_PROGRESS;
  process.env.ONI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "oni-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ONI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ONI_UPDATE_IN_PROGRESS;
  } else {
    process.env.ONI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
