...
const loadConfig = vi.fn(() => ({
  gateway: {
    mode: "remote",
    remote: { url: "wss://remote.example:19100", token: "rtok" },
    auth: { token: "ltok" },
  },
}));
const resolveGatewayPort = vi.fn((_cfg?: unknown) => 19100);
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    await withEnvAsync({ USER: "steipete" }, async () => {
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://peters-mac-studio-1.sheep-coho.ts.net:19100", token: "rtok" },
          auth: { token: "ltok" },
        },
      });
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    await withEnvAsync({ USER: "" }, async () => {
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "wss://studio.example:19100", token: "rtok" },
          auth: { token: "ltok" },
        },
      });
