...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.oni/workspace",
  applyWizardMetadata: (cfg: OniAIConfig) => cfg,
...
describe("runConfigureWizard", () => {
  it("persists gateway.mode=local when only the run mode is selected", async () => {
    ...
    mocks.resolveGatewayPort.mockReturnValue(19100);
    mocks.probeGatewayReachable.mockResolvedValue({ ok: false });
    mocks.resolveControlUiLinks.mockReturnValue({ wsUrl: "ws://127.0.0.1:19100" });
