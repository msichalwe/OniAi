...
type EnvSnapshot = {
  HOME?: string;
  ONI_HOME?: string;
  ONI_STATE_DIR?: string;
  ONI_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ONI_HOME: process.env.ONI_HOME,
    ONI_STATE_DIR: process.env.ONI_STATE_DIR,
    ONI_OAUTH_DIR: process.env.ONI_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OniAIConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OniAIConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "oni-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ONI_HOME = tempHome;
    process.env.ONI_STATE_DIR = path.join(tempHome, ".oni");
    delete process.env.ONI_OAUTH_DIR;
    fs.mkdirSync(process.env.ONI_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ONI_OAUTH_DIR is explicitly configured", async () => {
    process.env.ONI_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OniAIConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints oni-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OniAIConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/oni sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/oni sessions cleanup --store ".*sessions\.json" --dry-run/);
