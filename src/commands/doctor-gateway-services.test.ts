...
async function runRepair(cfg: OniAIConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
    ...
    issues: [
      {
        ...
        message: "Gateway service ONI_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      ONI_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses ONI_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ ONI_GATEWAY_TOKEN: "env-token" }, async () => {
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing OniAI gateway next.",
    );
  });
});
