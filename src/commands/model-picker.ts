...
type PromptDefaultModelResult = { model?: string; config?: OniAIConfig };
...
function resolveConfiguredModelRaw(cfg: OniAIConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OniAIConfig): string[] {
...
export function applyPrimaryModel(cfg: OniAIConfig, model: string): OniAIConfig {
...
export function applyModelAllowlist(cfg: OniAIConfig, models: string[]): OniAIConfig {
  const defaults = cfg.agents?.defaults;
