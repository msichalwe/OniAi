...
function resolveMode(cfg: OniAIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: OniAIConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("oni configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("oni config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("oni setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("oni doctor --fix")}" to apply changes.`);
  }
