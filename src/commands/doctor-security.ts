...
import type { OniAIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OniAIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("oni security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.oni/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("oni approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Example tunnel: ssh -N -L 19100:127.0.0.1:19100 user@gateway-host",
    "  Docs: https://docs.oni.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("oni configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("oni config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("oni doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("oni config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('oni config set session.dmScope "per-channel-peer"') +
