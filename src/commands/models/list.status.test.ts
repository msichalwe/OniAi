...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOniAIAgentDir: vi.fn().mockReturnValue("/tmp/oni-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/oni-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/oni-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOniAIAgentDir: mocks.resolveOniAIAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/oni-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOniAIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/oni-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/oni-agent-custom");
