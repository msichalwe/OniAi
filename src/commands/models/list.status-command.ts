...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOniAIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("oni models auth setup-token")}\` or \`${formatCliCommand("oni configure")}\`.`
          : `Run \`${formatCliCommand("oni configure")}\` or set an API key env var.`;
