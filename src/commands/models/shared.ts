...
export async function loadValidConfigOrThrow(): Promise<OniAIConfig> {
...
export async function updateConfig(
  mutator: (cfg: OniAIConfig) => OniAIConfig,
): Promise<OniAIConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: OniAIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OniAIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("oni agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
