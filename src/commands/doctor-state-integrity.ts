...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".oni"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg: OniAIConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.ONI_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".oni");
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`oni sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`oni sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
        ].join("\n"),
      );
    }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.oni out of git; it contains credentials and session history.",
