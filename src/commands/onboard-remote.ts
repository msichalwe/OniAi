...
const DEFAULT_GATEWAY_URL = "ws://127.0.0.1:19100";
...
function buildLabel(beacon: GatewayBonjourBeacon): string {
  ...
  const port = beacon.port ?? beacon.gatewayPort ?? 19100;
...
export async function promptRemoteGatewayConfig(
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.oni.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
  ...
  if (selectedBeacon) {
    ...
    const port = selectedBeacon.port ?? selectedBeacon.gatewayPort ?? 19100;
    if (host) {
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            `Using: ${suggestedUrl}`,
            "If your gateway is loopback-only, choose SSH tunnel and keep ws://127.0.0.1:19100.",
          ].join("\n"),
          "Direct remote",
        );
      } else {
