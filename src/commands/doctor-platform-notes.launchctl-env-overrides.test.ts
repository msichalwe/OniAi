...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ONI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ONI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ONI_GATEWAY_TOKEN");
    expect(message).not.toContain("ONI_GATEWAY_PASSWORD");
  });
