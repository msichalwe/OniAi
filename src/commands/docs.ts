...
const SEARCH_TOOL = "https://docs.oni.ai/mcp.SearchOniAI";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.oni.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('oni docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.oni.ai/");
      runtime.log(`Search: ${formatCliCommand('oni docs "your query"')}`);
    }
    return;
  }
