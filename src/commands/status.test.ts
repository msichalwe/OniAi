...
beforeAll(() => {
  envSnapshot = captureEnv(["ONI_PROFILE"]);
  process.env.ONI_PROFILE = "isolated";
});
...
vi.mock("../infra/oni-root.js", () => ({
  resolveOniAIPackageRoot: vi.fn().mockResolvedValue("/tmp/oni"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/oni/pnpm-lock.yaml",
      markerPath: "/tmp/oni/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.oni.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.oni.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("OniAI status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("oni status --all") ||
          l.includes("oni --profile isolated status --all") ||
          l.includes("oni status --all") ||
          l.includes("oni --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ONI_GATEWAY_TOKEN = "abcd1234";
    try {
