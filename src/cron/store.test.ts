...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ONI_HOME for tilde expansion", () => {
    vi.stubEnv("ONI_HOME", "/srv/oni-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/oni-home", "cron", "jobs.json"));
  });
});
