...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "oni-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ONI_GATEWAY_TOKEN: "test-token",
    ONI_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ONI_WORKSPACE_DIR: join(sandbox.rootDir, "oni"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ONI_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ONI_EXTRA_MOUNTS: undefined,
      ONI_HOME_VOLUME: "oni-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ONI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ONI_EXTRA_MOUNTS=");
    expect(envFile).toContain("ONI_HOME_VOLUME=oni-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("oni-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("oni-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ONI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ONI_CONFIG_DIR: configDir,
      ONI_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline ONI_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ONI_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ONI_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ONI_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ONI_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ONI_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ONI_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ONI_HOME_VOLUME must match");
  });
