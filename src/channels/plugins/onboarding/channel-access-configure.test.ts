...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OniAIConfig, policy: ChannelAccessPolicy) => OniAIConfig;
  resolveAllowlist: (params: { cfg: OniAIConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OniAIConfig; resolved: TResolved }) => OniAIConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OniAIConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OniAIConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OniAIConfig, policy: ChannelAccessPolicy): OniAIConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OniAIConfig, policy: ChannelAccessPolicy): OniAIConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OniAIConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OniAIConfig; resolved: string[] }) => {
      calls.push("apply");
