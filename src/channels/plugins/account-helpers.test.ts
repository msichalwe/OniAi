...
function cfg(accounts?: Record<string, unknown> | null): OniAIConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OniAIConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OniAIConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OniAIConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OniAIConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OniAIConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OniAIConfig)).toBe("default");
    });
  });
});
