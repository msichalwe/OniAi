...
function telegramCfg(): OniAIConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OniAIConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OniAIConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OniAIConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OniAIConfig;
...
function createSignalAccountOverrideCfg(): OniAIConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OniAIConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OniAIConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OniAIConfig,
        accountId: undefined,
        params: {
