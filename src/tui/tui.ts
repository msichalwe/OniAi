...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run oni devices list",
      pairingHint:
        "Pairing required. Run `oni devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `oni tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
  ...
  const { handleChatEvent, handleAgentEvent, handleInteractiveEvent } = createEventHandlers({
    chatLog,
    tui,
    state,
    setActivityStatus,
  ...
  client.onEvent = (evt) => {
    ...
    if (evt.event.startsWith("interactive.")) {
      handleInteractiveEvent(evt.event, evt.payload);
    }
  };
