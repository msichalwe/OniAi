...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ONI_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ONI_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ONI_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ONI_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
