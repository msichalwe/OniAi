...
function listAgents(cfg: OniAIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OniAIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OniAIConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OniAIConfig, EvaluatedBindingsCache>();
