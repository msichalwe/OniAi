...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(19100)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(19100)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(19100);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(19100, {
      timeoutMs: 500,
      intervalMs: 100,
