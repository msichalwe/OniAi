    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oni nodes status", "List known nodes with live status."],
          ["oni nodes pairing pending", "Show pending node pairing requests."],
          ['oni nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["oni nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.oni.ai/cli/nodes")}\n`,
    );
