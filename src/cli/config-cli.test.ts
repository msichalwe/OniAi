...
import type { ConfigFileSnapshot, OniAIConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `oni config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: OniAIConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OniAIConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: OniAIConfig, config: OniAIConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: OniAIConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
      ...
      expect(written.gateway?.port).toBe(19100);
