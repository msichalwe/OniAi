...
const resolveGatewayProgramArguments = vi.fn(async (_opts?: unknown) => ({
  programArguments: ["/bin/node", "cli", "gateway", "--port", "19100"],
}));
...
vi.mock("../infra/ports.js", () => ({
  ...
  formatPortDiagnostics: () => ["Port 19100 is already in use."],
}));
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ONI_STATE_DIR = "/tmp/oni-cli-state";
    process.env.ONI_CONFIG_PATH = "/tmp/oni-cli-state/oni.json";
    delete process.env.ONI_GATEWAY_PORT;
    delete process.env.ONI_PROFILE;
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ONI_PROFILE: "dev",
        ONI_STATE_DIR: "/tmp/oni-daemon-state",
        ONI_CONFIG_PATH: "/tmp/oni-daemon-state/oni.json",
        ONI_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.oni.gateway.plist",
    });
  ...
  it("installs the daemon (json output)", async () => {
    ...
    await runDaemonCommand(["daemon", "install", "--port", "19100", "--json"]);
