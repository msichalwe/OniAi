...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OniAI and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["oni update", "Update a source checkout (git)"],
        ["oni update --channel beta", "Switch to beta channel (git + npm)"],
        ["oni update --channel dev", "Switch to dev channel (git + npm)"],
        ["oni update --tag beta", "One-off update to a dist-tag or version"],
        ["oni update --dry-run", "Preview actions without changing anything"],
        ["oni update --no-restart", "Update without restarting the service"],
        ["oni update --json", "Output result as JSON"],
        ["oni update --yes", "Non-interactive (accept downgrade prompts)"],
        ["oni update wizard", "Interactive update wizard"],
        ["oni --update", "Shorthand for oni update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run oni update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.oni.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oni update status", "Show channel + version status."],
          ["oni update status --json", "JSON output."],
          ["oni update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.oni.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
