...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ONI_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ONI_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ONI_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("oni");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "oni", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "oni", "configure"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "oni", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "oni", "completion"],
    });
  ...
  it("hides banner when ONI_HIDE_BANNER is truthy", async () => {
    process.env.ONI_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "oni", "status"],
    });
