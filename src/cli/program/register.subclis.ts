...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.ONI_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.ONI_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<OniAIConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live OniAI docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "plugins",
    description: "Manage OniAI plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
