...
${formatHelpExamples([
  ['oni agent --to +15555550123 --message "status update"', "Start a new session."],
  ['oni agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'oni agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'oni agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['oni agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.oni.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.oni.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "oni agents set-identity --identity-file ~/.oni/workspace/IDENTITY.md --agent main",
