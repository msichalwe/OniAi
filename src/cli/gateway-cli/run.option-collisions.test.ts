...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/oni-test-missing-config.json",
  loadConfig: () => ({}),
...
vi.mock("../../gateway/auth.js", () => ({
  resolveGatewayAuth: (params: { authConfig?: { token?: string }; env?: NodeJS.ProcessEnv }) => ({
    mode: "token",
    token: params.authConfig?.token ?? params.env?.ONI_GATEWAY_TOKEN,
    password: undefined,
...
describe("gateway run option collisions", () => {
  ...
  it("forwards parent-captured options to `gateway run` subcommand", async () => {
    ...
    expect(forceFreePortAndWait).toHaveBeenCalledWith(19100, expect.anything());
