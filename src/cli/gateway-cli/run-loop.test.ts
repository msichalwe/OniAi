...
describe("runGatewayLoop", () => {
  ...
  it("forwards lockPort to initial and restart lock acquisitions", async () => {
    ...
    await withIsolatedSignals(async () => {
      ...
      await expect(loopPromise).rejects.toThrow("stop-loop");
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(1, { port: 19100 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(2, { port: 19100 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(3, { port: 19100 });
    });
  });
...
describe("gateway discover routing helpers", () => {
  ...
  it("prefers resolved service port over TXT gatewayPort", () => {
    ...
    expect(pickGatewayPort(beacon)).toBe(19100);
  });
