...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "oni-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "oni-bundled-skills-test-"));
    process.env.ONI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
