    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oni memory status", "Show index and provider status."],
          ["oni memory index --force", "Force a full reindex."],
          ['oni memory search --query "deployment notes"', "Search indexed memory entries."],
          ["oni memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.oni.ai/cli/memory")}\n`,
    );
