...
const SANDBOX_EXAMPLES = {
  main: [
    ["oni sandbox list", "List all sandbox containers."],
    ["oni sandbox list --browser", "List only browser containers."],
    ["oni sandbox recreate --all", "Recreate all containers."],
    ["oni sandbox recreate --session main", "Recreate a specific session."],
    ["oni sandbox recreate --agent mybot", "Recreate agent containers."],
    ["oni sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["oni sandbox list", "List all sandbox containers."],
    ["oni sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["oni sandbox recreate --all", "Recreate all containers."],
    ["oni sandbox recreate --session main", "Recreate a specific session."],
    ["oni sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["oni sandbox recreate --browser --all", "Recreate only browser containers."],
    ["oni sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["oni sandbox explain", "Show effective sandbox config."],
    ["oni sandbox explain --session agent:main:main", "Explain a specific session."],
    ["oni sandbox explain --agent work", "Explain an agent sandbox."],
    ["oni sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.oni.ai/cli/sandbox")}\n`,
    )
    .action(() => {
