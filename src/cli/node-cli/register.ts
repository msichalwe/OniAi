...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "oni node run --host 127.0.0.1 --port 19100",
            "Run the node host in the foreground.",
          ],
          ["oni node status", "Check node host service status."],
          ["oni node install", "Install the node host service."],
          ["oni node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.oni.ai/cli/node")}\n`,
    );
  ...
  node
    ...
    .action(async (opts) => {
      ...
      const port = parsePortWithFallback(opts.port, existing?.gateway?.port ?? 19100);
      await runNodeHost({
        gatewayHost: host,
        gatewayPort: port,
