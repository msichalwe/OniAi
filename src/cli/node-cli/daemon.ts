...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("oni node install"), formatCliCommand("oni node start")];
...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 19100;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("oni node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ONI_LOG_PREFIX: baseEnv.ONI_LOG_PREFIX ?? "node",
