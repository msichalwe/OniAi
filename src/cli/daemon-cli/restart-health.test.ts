...
describe("inspectGatewayRestart", () => {
  ...
  it("treats a gateway listener child pid as healthy ownership", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      port: 19100,
      status: "busy",
      listeners: [{ pid: 7001, ppid: 7000, commandLine: "oni-gateway" }],
      hints: [],
    });

    const { inspectGatewayRestart } = await import("./restart-health.js");
    const snapshot = await inspectGatewayRestart({ service, port: 19100 });
  ...
  it("marks non-owned gateway listener pids as stale while runtime is running", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      port: 19100,
      status: "busy",
      listeners: [{ pid: 9000, ppid: 8999, commandLine: "oni-gateway" }],
      hints: [],
    });
