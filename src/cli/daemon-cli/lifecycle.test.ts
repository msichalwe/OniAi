...
const resolveGatewayPort = vi.fn(() => 19100);
...
describe("runDaemonRestart health checks", () => {
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["oni", "gateway", "--port", "19100"],
      environment: {},
    });
  ...
  it("kills stale gateway pids and retries restart", async () => {
    const unhealthy: RestartHealthSnapshot = {
      ...
      portUsage: { port: 19100, status: "busy", listeners: [], hints: [] },
    };
  ...
  it("fails restart when gateway remains unhealthy", async () => {
    const unhealthy: RestartHealthSnapshot = {
      ...
      staleGatewayPids: [],
      runtime: { status: "stopped" },
      portUsage: { port: 19100, status: "free", listeners: [], hints: [] },
    };
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toMatchObject({
      ...
      hints: ["oni gateway status --deep", "oni doctor"],
    });
