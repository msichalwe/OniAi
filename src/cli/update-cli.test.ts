...
import type { OniAIConfig, ConfigFileSnapshot } from "../config/types.oni.js";
...
vi.mock("../infra/oni-root.js", () => ({
  resolveOniAIPackageRoot: vi.fn(),
}));

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: vi.fn(),
  resolveGatewayPort: vi.fn(() => 19100),
...
const { resolveOniAIPackageRoot } = await import("../infra/oni-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/oni-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OniAIConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/oni-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOniAIPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("oni-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOniAIPackageRoot).mockClear();
    ...
    vi.mocked(resolveOniAIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("oni");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/oni-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 19100,
      status: "busy",
      listeners: [{ pid: 4242, command: "oni-gateway" }],
      hints: [],
    });
    classifyPortListener.mockReturnValue("gateway");
    formatPortDiagnostics.mockReturnValue(["Port 19100 is already in use."]);
    vi.mocked(runDaemonInstall).mockResolvedValue(undefined);
    vi.mocked(runDaemonRestart).mockResolvedValue(true);
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OniAIConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOniAIPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("oni-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ONI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ONI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("oni-update-wizard");
    await withEnvAsync({ ONI_GIT_DIR: tempDir }, async () => {
      setTty(true);
