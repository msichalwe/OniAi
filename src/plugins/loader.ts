...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OniAIPluginDefinition;
  register?: OniAIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OniAIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OniAIPluginDefinition;
...
export function loadOniAIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOniAIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "oni/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "oni/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OniAIPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as OniAIPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
