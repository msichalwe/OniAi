...
import { loadOniAIPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `oni-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ONI_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.ONI_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOniAIPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadOniAIPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ONI_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ONI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOniAIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOniAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOniAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.ONI_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ ONI_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
