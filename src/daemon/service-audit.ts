...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ONI_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ONI_GATEWAY_TOKEN does not match gateway.auth.token in oni.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `oni gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
