...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ONI_GATEWAY_PORT).toBe("19100");
    expect(env.ONI_GATEWAY_TOKEN).toBe("secret");
    expect(env.ONI_SERVICE_MARKER).toBe("oni");
    expect(env.ONI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ONI_SERVICE_VERSION).toBe("string");
    expect(env.ONI_SYSTEMD_UNIT).toBe("oni-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ONI_LAUNCHD_LABEL).toBe("ai.oni.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ONI_PROFILE: "work" },
      port: 19100,
    });
    expect(env.ONI_SYSTEMD_UNIT).toBe("oni-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ONI_LAUNCHD_LABEL).toBe("ai.oni.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oni"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ONI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oni-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ONI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oni"));
  });

  it("uses ONI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ONI_STATE_DIR: "/var/lib/oni" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/oni"));
  });

  it("expands ~ in ONI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ONI_STATE_DIR: "~/oni-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/oni-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ONI_STATE_DIR: "C:\\State\\oni" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\oni");
  });
});
