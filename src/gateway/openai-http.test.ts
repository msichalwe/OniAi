    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "oni", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-oni-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "oni", messages: [{ role: "user", content: "hi" }] },
          {
            "x-oni-agent-id": "beta",
            "x-oni-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from OniAI.");
      }
