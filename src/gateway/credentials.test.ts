...
function cfg(input: Partial<OniAIConfig>): OniAIConfig {
  return input as OniAIConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<OniAIConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  ONI_GATEWAY_TOKEN: "env-token",
  ONI_GATEWAY_PASSWORD: "env-password",
