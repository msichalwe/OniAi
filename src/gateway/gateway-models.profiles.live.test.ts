...
import { resolveOniAIAgentDir } from "../agents/agent-paths.js";
...
import { ensureOniAIModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, OniAIConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.ONI_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ONI_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ONI_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ONI_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OniAIConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.ONI_CONFIG_PATH,
    token: process.env.ONI_GATEWAY_TOKEN,
    skipChannels: process.env.ONI_SKIP_CHANNELS,
    skipGmail: process.env.ONI_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ONI_SKIP_CRON,
    skipCanvas: process.env.ONI_SKIP_CANVAS_HOST,
    agentDir: process.env.ONI_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ONI_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.ONI_SKIP_CHANNELS = "1";
  process.env.ONI_SKIP_GMAIL_WATCHER = "1";
  process.env.ONI_SKIP_CRON = "1";
  process.env.ONI_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.ONI_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveOniAIAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-live-state-"));
  process.env.ONI_STATE_DIR = tempStateDir;
  ...
  process.env.ONI_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.oni-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOniAIAgentDir();
  const sanitizedCfg: OniAIConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-live-"));
  const tempConfigPath = path.join(tempDir, "oni.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ONI_CONFIG_PATH = tempConfigPath;

  await ensureOniAIModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "OniAI live tool probe (local, safe): " +
    ...
    process.env.ONI_CONFIG_PATH = previous.configPath;
    process.env.ONI_GATEWAY_TOKEN = previous.token;
    process.env.ONI_SKIP_CHANNELS = previous.skipChannels;
    process.env.ONI_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ONI_SKIP_CRON = previous.skipCron;
    process.env.ONI_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ONI_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ONI_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOniAIModelsJson(cfg);
      ...
      const rawModels = process.env.ONI_LIVE_GATEWAY_MODELS?.trim();
      const useModern = !rawModels || rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseFilter(rawModels) : null;
      const maxModels = toInt(process.env.ONI_LIVE_GATEWAY_MAX_MODELS, 0);
      const wanted = filter
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ONI_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.oni-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
