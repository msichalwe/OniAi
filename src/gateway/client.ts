...
export class GatewayClient {
  ...
  start() {
    ...
    const url = this.opts.url ?? "ws://127.0.0.1:19100";
    ...
    if (!isSecureWebSocketUrl(url)) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          "(ssh -N -L 19100:127.0.0.1:19100 user@gateway-host), or use Tailscale Serve/Funnel. " +
          "Run `oni doctor --fix` for guidance.",
      );
