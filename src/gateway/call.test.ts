...
function setGatewayNetworkDefaults(port = 19100) {
  resolveGatewayPort.mockReturnValue(port);
  pickPrimaryTailnetIPv4.mockReturnValue(undefined);
}

function setLocalLoopbackGatewayConfig(port = 19100) {
...
function makeRemotePasswordGatewayConfig(remotePassword: string, localPassword = "from-config") {
  return {
    gateway: {
      mode: "remote",
      remote: { url: "wss://remote.example:19100", password: remotePassword },
...
describe("callGateway url resolution", () => {
  ...
  it("uses url override in remote mode even when remote url is missing", async () => {
    ...
    resolveGatewayPort.mockReturnValue(19100);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("emits a remote fallback note when remote url is missing", () => {
    ...
    expect(details.url).toBe("ws://127.0.0.1:19100");
    ...
    expect(details.message).toContain("Gateway target: ws://127.0.0.1:19100");
  });
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://remote.example.com:19100" },
      },
    });
    resolveGatewayPort.mockReturnValue(19100);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    ...
    expect((thrown as Error).message).toContain("oni doctor --fix");
  });
...
describe("callGateway error details", () => {
  ...
  it("includes connection details when the gateway closes", async () => {
    ...
    expect(err?.message).toContain("Gateway target: ws://127.0.0.1:19100");
  ...
  it("includes connection details on timeout", async () => {
    ...
    expect(errMessage).toContain("Gateway target: ws://127.0.0.1:19100");
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ONI_GATEWAY_TOKEN", "ONI_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
    setGatewayNetworkDefaults(19100);
  });
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.ONI_GATEWAY_TOKEN = "env-token";
    process.env.ONI_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ONI_GATEWAY_PASSWORD", "ONI_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ONI_GATEWAY_PASSWORD = envPassword;
    }
