...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ONI_STATE_DIR",
  "ONI_CONFIG_PATH",
  "ONI_SKIP_BROWSER_CONTROL_SERVER",
  "ONI_SKIP_GMAIL_WATCHER",
  "ONI_SKIP_CANVAS_HOST",
  "ONI_BUNDLED_PLUGINS_DIR",
  "ONI_SKIP_CHANNELS",
  "ONI_SKIP_PROVIDERS",
  "ONI_SKIP_CRON",
  "ONI_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "oni-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ONI_STATE_DIR = path.join(tempHome, ".oni");
  delete process.env.ONI_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ONI_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ONI_SKIP_GMAIL_WATCHER = "1";
  process.env.ONI_SKIP_CANVAS_HOST = "1";
  process.env.ONI_SKIP_CHANNELS = "1";
  process.env.ONI_SKIP_PROVIDERS = "1";
  process.env.ONI_SKIP_CRON = "1";
  process.env.ONI_TEST_MINIMAL_GATEWAY = "1";
  process.env.ONI_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "oni-test-no-bundled-extensions")
    : "oni-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "oni-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".oni-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__oniTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__oniTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ONI_GATEWAY_TOKEN"]);
  const prev = process.env.ONI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ONI_GATEWAY_TOKEN = fallbackToken;
  }
