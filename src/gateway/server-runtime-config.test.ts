...
describe("resolveGatewayRuntimeConfig", () => {
  describe("trusted-proxy auth mode", () => {
    ...
    it.each([
      ...
      const result = await resolveGatewayRuntimeConfig({ cfg, port: 19100 });
    ...
    it.each([
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, port: 19100 })).rejects.toThrow(
        expectedMessage,
      );
    });
  });
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.ONI_GATEWAY_TOKEN;
      delete process.env.ONI_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.ONI_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.ONI_GATEWAY_TOKEN;
      }
    });
    ...
    it.each([
      {
        ...
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or ONI_GATEWAY_TOKEN)",
      },
      {
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, port: 19100, host })).rejects.toThrow(
        expectedMessage,
      );
    });
