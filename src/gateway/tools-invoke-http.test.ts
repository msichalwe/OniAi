...
let lastCreateOniAIToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/oni-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOniAIToolsContext?.agentTo,
          agentThreadId: lastCreateOniAIToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOniAITools: (ctx: Record<string, unknown>) => {
      lastCreateOniAIToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOniAIToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-oni-message-to": "channel:24514",
        "x-oni-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
