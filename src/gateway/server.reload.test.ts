...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ONI_SKIP_CHANNELS;
    prevSkipGmail = process.env.ONI_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ONI_SKIP_PROVIDERS;
    process.env.ONI_SKIP_CHANNELS = "0";
    delete process.env.ONI_SKIP_GMAIL_WATCHER;
    delete process.env.ONI_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ONI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ONI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ONI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ONI_SKIP_PROVIDERS;
    } else {
      process.env.ONI_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
