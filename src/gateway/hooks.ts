...
export function resolveHooksConfig(cfg: OniAIConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OniAIConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-oni-token"] === "string"
      ? req.headers["x-oni-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
