...
const { createOniAITools } = await import("../agents/oni-tools.js");
...
type SessionSendTool = ReturnType<typeof createOniAITools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOniAITools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ONI_GATEWAY_PORT", "ONI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ONI_GATEWAY_PORT = String(gatewayPort);
  process.env.ONI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.ONI_CONFIG_PATH;
      if (!configPath) {
        throw new Error("ONI_CONFIG_PATH missing in gateway test environment");
      }
