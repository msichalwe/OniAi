...
import { createOniAITools } from "../agents/oni-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-oni-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-oni-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-oni-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-oni-thread-id")?.trim() || undefined;
  ...
  const allTools = createOniAITools({
