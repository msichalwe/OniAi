...
declare const __ONI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "oni";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ONI_VERSION"],
      env["ONI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current OniAI version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __ONI_VERSION__ === "string" && __ONI_VERSION__) ||
  process.env.ONI_BUNDLED_VERSION ||
