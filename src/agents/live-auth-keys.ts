...
const GOOGLE_LIVE_SINGLE_KEY = "ONI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ONI_LIVE_ANTHROPIC_KEY",
    listVar: "ONI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ONI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ONI_LIVE_${base}_KEY`;
