...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("oni-ws-", async (workspaceDir) => {
      await withTempDir("oni-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createOniAICodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("oni-ws-", async (workspaceDir) => {
      await withTempDir("oni-override-", async (overrideDir) => {
        const tools = createOniAICodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("oni-ws-", async (workspaceDir) => {
      const cfg: OniAIConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createOniAICodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-oni.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("oni-sandbox-", async (sandboxDir) => {
      await withTempDir("oni-workspace-", async (workspaceDir) => {
        ...
        const tools = createOniAICodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
