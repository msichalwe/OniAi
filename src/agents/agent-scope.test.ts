...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OniAIConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OniAIConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OniAIConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OniAIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OniAIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OniAIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/oni");
  });

  it("uses ONI_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "oni-home");
    vi.stubEnv("ONI_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OniAIConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".oni", "workspace"));
  });

  it("uses ONI_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "oni-home");
    vi.stubEnv("ONI_HOME", home);
    // Clear state dir so it falls back to ONI_HOME
    vi.stubEnv("ONI_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OniAIConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".oni", "agents", "main", "agent"));
  });
});
