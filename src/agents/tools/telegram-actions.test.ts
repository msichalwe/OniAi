...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OniAIConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OniAIConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OniAIConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OniAIConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: OniAIConfig, accountId = "media") {
    await handleTelegramAction(
