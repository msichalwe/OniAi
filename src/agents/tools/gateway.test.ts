...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    oni: process.env.ONI_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.oni === undefined) {
      delete process.env.ONI_GATEWAY_TOKEN;
    } else {
      process.env.ONI_GATEWAY_TOKEN = envSnapshot.oni;
    }
  ...
  it("accepts allowlisted gatewayUrl overrides (SSRF hardening)", async () => {
    ...
    await callGatewayTool(
      "health",
      { gatewayUrl: "ws://127.0.0.1:19100", gatewayToken: "t", timeoutMs: 5000 },
      {},
    );
  ...
  it("uses ONI_GATEWAY_TOKEN for allowlisted local overrides", () => {
    process.env.ONI_GATEWAY_TOKEN = "env-token";
    const opts = resolveGatewayOptions({ gatewayUrl: "ws://127.0.0.1:19100" });
    expect(opts.url).toBe("ws://127.0.0.1:19100");
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ONI_GATEWAY_TOKEN = "local-env-token";
