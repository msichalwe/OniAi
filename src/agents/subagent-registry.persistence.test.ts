...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ONI_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-subagent-"));
  ...
  it("uses isolated temp state when ONI_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "oni-test-state"));
  });
});
