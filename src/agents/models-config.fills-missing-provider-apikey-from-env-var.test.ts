...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureOniAIModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureOniAIModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
  ...
  it("refreshes stale explicit moonshot model capabilities from implicit catalog", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        const modelPath = path.join(resolveOniAIAgentDir(), "models.json");
