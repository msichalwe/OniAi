...
const bundledPluginsDirSnapshot = captureEnv(["ONI_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.ONI_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
const { createOniAICodingTools } = await import("./pi-tools.js");
...
describe("createOniAICodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oni-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oni-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oni-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oni-safe-bins-redirect-",
        safeBins: ["head"],
