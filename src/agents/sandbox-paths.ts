...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const oniTmpDir = path.resolve(resolvePreferredOniAITmpDir());
  if (!isPathInside(oniTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: oniTmpDir });
  return resolved;
}
