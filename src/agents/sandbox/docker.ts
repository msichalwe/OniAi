...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "oni.sandbox=1");
  args.push("--label", `oni.sessionKey=${params.scopeKey}`);
  args.push("--label", `oni.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `oni.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "oni.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`oni sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`oni sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("oni sandbox recreate --all");
}
