...
import { createOniAICodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OniAIConfig) {
    return createOniAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OniAIConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OniAIConfig["agents"]>["list"]>[number]["tools"];
  }): OniAIConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOniAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOniAICodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOniAICodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOniAICodingTools({
      config: cfg,
    ...
    const bobTools = createOniAICodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOniAICodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOniAICodingTools({
      config: cfg,
