...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ONI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ONI_HOME: "/srv/oni-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/oni-home"), ".oni", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".oni", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("oni-workspace-");
