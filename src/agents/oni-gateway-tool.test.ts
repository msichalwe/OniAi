...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oni-test-"));

    try {
      await withEnvAsync(
        { ONI_STATE_DIR: stateDir, ONI_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: oni --profile isolated doctor --non-interactive",
          );
