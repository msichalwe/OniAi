...
type AgentEntry = NonNullable<NonNullable<OniAIConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: OniAIConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OniAIConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OniAIConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: OniAIConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: OniAIConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: OniAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: OniAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
