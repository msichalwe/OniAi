...
import { resolveOniAIDocsPath } from "../../docs-path.js";
...
import { createOniAICodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveOniAIAgentDir();
    ...
    const docsPath = await resolveOniAIDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/oni/oni/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt") {
          try {
            sessionManager.appendCustomEntry("oni:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/oni/oni/issues/8643
