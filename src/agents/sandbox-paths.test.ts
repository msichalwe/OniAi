...
import { resolvePreferredOniAITmpDir } from "../infra/tmp-oni-dir.js";
...
async function withOutsideHardlinkInOniAITmp<T>(
  params: {
    oniTmpDir: string;
  ...
  const hardlinkPath = path.join(params.oniTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.oniTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.oniTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.oniTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const oniTmpDir = resolvePreferredOniAITmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred OniAI tmp root",
      media: path.join(oniTmpDir, "image.png"),
      expected: path.join(oniTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred OniAI tmp root",
      media: pathToFileURL(path.join(oniTmpDir, "photo.png")).href,
      expected: path.join(oniTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred OniAI tmp root",
      media: path.join(oniTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(oniTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(oniTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside oni tmp root",
      media: path.join(os.tmpdir(), "outside-oni", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked OniAI tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(oniTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(oniTmpDir, { recursive: true });
      const symlinkPath = path.join(oniTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked OniAI tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOniAITmp(
      {
        oniTmpDir,
  ...
  it("rejects symlinked OniAI tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOniAITmp(
