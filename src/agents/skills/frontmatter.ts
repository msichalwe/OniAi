...
import {
  ...
  normalizeStringList,
  parseOniAIManifestInstallBase,
  parseFrontmatterBool,
  resolveOniAIManifestBlock,
  resolveOniAIManifestInstall,
  resolveOniAIManifestOs,
  resolveOniAIManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOniAIManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveOniAIMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OniAISkillMetadata | undefined {
  const metadataObj = resolveOniAIManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveOniAIManifestRequires(metadataObj);
  const install = resolveOniAIManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveOniAIManifestOs(metadataObj);
  return {
