...
function resolveSkillsLimits(config?: OniAIConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOniAIMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OniAIConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`oni skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
