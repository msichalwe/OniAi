...
const defaultTools = createOniAICodingTools({ senderIsOwner: true });
...
describe("createOniAICodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OniAIConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOniAICodingTools({
    ...
    const anthropicTools = createOniAICodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OniAIConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOniAICodingTools({
    ...
    const denied = createOniAICodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOniAICodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
