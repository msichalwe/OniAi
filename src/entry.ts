...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "oni.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "oni.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ONI_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ONI_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ONI_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[oni] ${parsed.error}`);
      process.exit(2);
    }
