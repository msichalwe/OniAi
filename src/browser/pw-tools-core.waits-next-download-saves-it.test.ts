...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOniAITmpDir: vi.fn(() => "/tmp/oni"),
}));
vi.mock("../infra/tmp-oni-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOniAITmpDir.mockReturnValue("/tmp/oni");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOniAITmpDir.mockReturnValue("/tmp/oni-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "oni-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOniAITmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "oni-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "oni-preferred", "downloads")}${path.sep}`),
    );
  });
