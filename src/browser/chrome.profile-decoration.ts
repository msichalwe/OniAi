...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".oni-profile-decorated");
}
 ...
 */
export function decorateOniAIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ONI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ONI_BROWSER_COLOR).toUpperCase();
