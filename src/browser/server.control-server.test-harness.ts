...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ONI_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/oni" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "oni",
        profiles: {
          oni: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOniAIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.ONI_GATEWAY_PORT;
  process.env.ONI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ONI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ONI_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ONI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ONI_GATEWAY_PASSWORD;
  } else {
    process.env.ONI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
