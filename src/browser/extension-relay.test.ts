...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ONI_GATEWAY_TOKEN"]);
    process.env.ONI_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-oni-relay-token");
    expect(headers["x-oni-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-oni-relay-token"];
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${cdpUrl}/json/version`, {
      headers: { "x-oni-relay-token": TEST_GATEWAY_TOKEN },
    });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-oni-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "OniAI/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an oni relay", async () => {
    const port = await getFreePort();
