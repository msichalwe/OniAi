import type { BrowserConfig, BrowserProfileConfig, OniAIConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "oni" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ONI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "oni" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ONI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ONI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OniAI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for oni/oni CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OniAIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ONI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "oni";
