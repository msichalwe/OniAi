  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "oni",
        profiles: {
          oni: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ONI_GATEWAY_PORT;
    process.env.ONI_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ONI_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ONI_GATEWAY_PASSWORD;
