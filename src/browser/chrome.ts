...
import {
  decorateOniAIProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";
import { DEFAULT_ONI_BROWSER_COLOR, DEFAULT_ONI_BROWSER_PROFILE_NAME } from "./constants.js";
...
export function resolveOniAIUserDataDir(profileName = DEFAULT_ONI_BROWSER_PROFILE_NAME) {
...
export async function launchOniAIChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ONI_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOniAIProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`oni browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopOniAIChrome(running: RunningChrome, timeoutMs = 2500) {
