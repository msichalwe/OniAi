...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("oni-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("oni-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("oni-lid-a-", (first) => {
      withTempDirSync("oni-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.oni when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "oni-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ONI_HOME over HOME", () => {
    vi.stubEnv("ONI_HOME", "/srv/oni-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/oni-home"));
...
describe("shortenHomePath", () => {
  it("uses $ONI_HOME prefix when ONI_HOME is set", () => {
    vi.stubEnv("ONI_HOME", "/srv/oni-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/oni-home")}/.oni/oni.json`)).toBe(
      "$ONI_HOME/.oni/oni.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ONI_HOME replacement when ONI_HOME is set", () => {
    vi.stubEnv("ONI_HOME", "/srv/oni-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/oni-home")}/.oni/oni.json`),
    ).toBe("config: $ONI_HOME/.oni/oni.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/oni")).toBe(path.resolve(os.homedir(), "oni"));
  });
  ...
  it("prefers ONI_HOME for tilde expansion", () => {
    vi.stubEnv("ONI_HOME", "/srv/oni-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/oni")).toBe(path.resolve("/srv/oni-home", "oni"));
