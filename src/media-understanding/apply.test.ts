...
const TEMP_MEDIA_PREFIX = "oni-media-";
...
function createGroqAudioConfig(): OniAIConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OniAIConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OniAIConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OniAIConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOniAITmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ONI_AGENT_DIR: isolatedAgentDir,
