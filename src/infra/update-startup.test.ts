...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ONI_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ONI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOniAIPackageRoot } = await import("./oni-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveOniAIPackageRoot).mockResolvedValue("/opt/oni");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/oni/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
