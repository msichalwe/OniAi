...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`$ORIGIN oni.internal.`);
    ...
    expect(txt).toContain(`_oni-gw._tcp IN PTR studio-london._oni-gw._tcp`);
    expect(txt).toContain(`studio-london._oni-gw._tcp IN SRV 0 0 19100 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (OniAI)`);
    expect(txt).toContain(`gatewayPort=19100`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/oni`);
  });
