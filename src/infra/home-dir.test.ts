...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ONI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands ONI_HOME when set to ~", () => {
    const env = {
      ONI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ONI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ONI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ONI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ONI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ONI_HOME: "/srv/oni-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/oni-home")}/x`);
  });
