...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ONI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*oni-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "oni";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "oni");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "oni-gw");
  ...
  records.push(`_oni-gw._tcp IN PTR ${instanceLabel}._oni-gw._tcp`);
  records.push(`${instanceLabel}._oni-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._oni-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; oni-content-hash: ${contentHash}\n${contentBody}`;
}
