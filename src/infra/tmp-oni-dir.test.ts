...
import { POSIX_ONI_TMP_DIR, resolvePreferredOniAITmpDir } from "./tmp-oni-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOniAITmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `oni-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOniAITmpDir({
    accessSync,
...
describe("resolvePreferredOniAITmpDir", () => {
  it("prefers /tmp/oni when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ONI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/oni when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ONI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/oni when /tmp/oni is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/oni when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/oni is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/oni is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/oni is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
