...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "oni-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("oni-plugin-", expect.any(Function));
  ...
  it("returns extract failure when extraction throws", async () => {
    vi.spyOn(installSource, "withTempDir").mockImplementation(
      async (_prefix, fn) => await fn("/tmp/oni-install-flow"),
    );
