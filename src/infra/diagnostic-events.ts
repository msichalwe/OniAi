...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __oniDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__oniDiagnosticEventsState) {
    globalStore.__oniDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__oniDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OniAIConfig): boolean {
