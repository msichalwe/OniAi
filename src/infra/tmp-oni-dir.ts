...
export const POSIX_ONI_TMP_DIR = "/tmp/oni";

type ResolvePreferredOniAITmpDirOptions = {
...
export function resolvePreferredOniAITmpDir(
  options: ResolvePreferredOniAITmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "oni" : `oni-${uid}`;
  ...
  const resolvePreferredState = (
    ...
    try {
      const preferred = lstatSync(POSIX_ONI_TMP_DIR);
      if (!isTrustedPreferredDir(preferred)) {
        return "invalid";
      }
      if (requireWritableAccess) {
        accessSync(POSIX_ONI_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
      }
      return "available";
    } catch (err) {
  ...
  try {
    ...
    mkdirSync(POSIX_ONI_TMP_DIR, { recursive: true, mode: 0o700 });
