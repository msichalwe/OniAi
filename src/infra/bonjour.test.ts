...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.ONI_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.gatewayPort).toBe("19100");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("oni (OniAI)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("oni");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("oni.local");
