...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__oni_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "oni-root");
...
describe("resolveOniAIPackageRoot", () => {
  let resolveOniAIPackageRoot: typeof import("./oni-root.js").resolveOniAIPackageRoot;
  let resolveOniAIPackageRootSync: typeof import("./oni-root.js").resolveOniAIPackageRootSync;

  beforeAll(async () => {
    ({ resolveOniAIPackageRoot, resolveOniAIPackageRootSync } =
      await import("./oni-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "oni");
    const pkgRoot = path.join(project, "node_modules", "oni");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "oni" }));

    expect(resolveOniAIPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "oni");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "oni.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "oni" }));

    expect(resolveOniAIPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOniAIPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-oni package roots", async () => {
    const pkgRoot = fx("not-oni");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-oni" }));

    expect(resolveOniAIPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOniAIPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOniAIPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
