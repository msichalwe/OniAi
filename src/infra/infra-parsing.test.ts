...
describe("infra parsing", () => {
  ...
  describe("isMainModule", () => {
    ...
    it("returns true for dist/entry.js when launched via oni.mjs wrapper", () => {
      expect(
        isMainModule({
          currentFile: "/repo/dist/entry.js",
          argv: ["node", "/repo/oni.mjs"],
          cwd: "/repo",
          env: {},
          wrapperEntryPairs: [{ wrapperBasename: "oni.mjs", entryBasename: "entry.js" }],
        }),
      ).toBe(true);
    });
    ...
    it("returns false when running under PM2 but this module is imported", () => {
      expect(
        isMainModule({
          currentFile: "/repo/node_modules/oni/dist/index.js",
