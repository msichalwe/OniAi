...
vi.mock("./oni-root.js", () => ({
  resolveOniAIPackageRoot: vi.fn(async () => null),
  resolveOniAIPackageRootSync: vi.fn(() => null),
}));
...
let oniRoot: typeof import("./oni-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    oniRoot = await import("./oni-root.js");
  });
  ...
  it("uses resolveOniAIPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/oni");
    (
      oniRoot.resolveOniAIPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/oni"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "oni" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "oni.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-oni");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/oni-bundle");
    (
      oniRoot.resolveOniAIPackageRootSync as unknown as ReturnType<typeof vi.fn>
