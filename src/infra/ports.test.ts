...
describe("ports helpers", () => {
  ...
  it("prints an OniAI-specific hint when port details look like another OniAI instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(19100, "node dist/index.js oni gateway"),
      19100,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another OniAI instance is already running");
  });

  it("classifies ssh and gateway listeners", () => {
    expect(
      classifyPortListener({ commandLine: "ssh -N -L 19100:127.0.0.1:19100 user@host" }, 19100),
    ).toBe("ssh");
    expect(
      classifyPortListener(
        {
          commandLine: "node /Users/me/Projects/oni/dist/entry.js gateway",
        },
        19100,
      ),
    ).toBe("gateway");
  ...
  it("formats port diagnostics with hints", () => {
    const diagnostics = {
      ...
      listeners: [{ pid: 123, commandLine: "ssh -N -L 19100:127.0.0.1:19100" }],
      hints: buildPortHints([{ pid: 123, commandLine: "ssh -N -L 19100:127.0.0.1:19100" }], 19100),
    };
    const lines = formatPortDiagnostics(diagnostics);
    expect(lines[0]).toContain("Port 19100 is already in use");
