...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OniAIConfig | null = null;
  ...
  get(key: string, cfg: OniAIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OniAIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OniAIConfig): void {
