...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred OniAI tmp root", async () => {
    const tmpRoot = resolvePreferredOniAITmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOniAI = path.join(os.tmpdir(), "outside-oni", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
