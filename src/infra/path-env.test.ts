...
let ensureOniAICliOnPath: typeof import("./path-env.js").ensureOniAICliOnPath;

describe("ensureOniAICliOnPath", () => {
  const envKeys = [
    "PATH",
    "ONI_PATH_BOOTSTRAPPED",
    "ONI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOniAICliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling oni exists", () => {
    const tmp = abs("/tmp/oni-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "oni");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ONI_PATH_BOOTSTRAPPED;

    ensureOniAICliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ONI_PATH_BOOTSTRAPPED = "1";
    ensureOniAICliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/oni-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "oni");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/oni-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "oni");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "oni");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/oni-path/case-linuxbrew");
