...
const DEFAULT_PACKAGE_NAME = "oni";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oni-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "oni.mjs");
    ...
    const doctorStep = await runStep(
      step("oni doctor", doctorArgv, gitRoot, { ONI_UPDATE_IN_PROGRESS: "1" }),
    );
