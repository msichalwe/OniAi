...
export function triggerOniAIRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ONI_SYSTEMD_UNIT,
        process.env.ONI_PROFILE,
      );
  ...
  const label =
    process.env.ONI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ONI_PROFILE);
