...
function candidateBinDirs(opts: EnsureOniAIPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `oni` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "oni");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ONI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "oni"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `oni` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOniAICliOnPath(opts: EnsureOniAIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ONI_PATH_BOOTSTRAPPED)) {
    return;
  }
