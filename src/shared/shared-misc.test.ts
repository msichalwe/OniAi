...
describe("shared/frontmatter", () => {
  ...
  test("resolveOniAIManifestBlock parses JSON5 metadata and picks oni block", () => {
    const frontmatter = {
      metadata: "{ oni: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOniAIManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOniAIManifestBlock returns undefined for invalid input", () => {
    expect(resolveOniAIManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOniAIManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOniAIManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
