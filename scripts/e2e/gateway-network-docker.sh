...
IMAGE_NAME="oni-gateway-network-e2e"

PORT="19100"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="oni-net-e2e-$$"
GW_NAME="oni-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ONI_GATEWAY_TOKEN=$TOKEN" \
  -e "ONI_SKIP_CHANNELS=1" \
  -e "ONI_SKIP_GMAIL_WATCHER=1" \
  -e "ONI_SKIP_CRON=1" \
  -e "ONI_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
