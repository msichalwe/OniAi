...
# Build and bundle OniAI into a minimal .app we can open.
# Outputs to dist/OniAI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OniAI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OniAI"
BUNDLE_ID="${BUNDLE_ID:-ai.oni.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/oni/oni/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OniAI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OniAIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OniAIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OniAI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OniAI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OniAI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OniAI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OniAI/Resources/OniAI.icns" "$APP_ROOT/Contents/Resources/OniAI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OniAI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OniAIKit resources"
ONIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OniAIKit_OniAIKit.bundle"
if [ -d "$ONIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OniAIKit_OniAIKit.bundle"
  cp -R "$ONIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OniAIKit_OniAIKit.bundle"
else
  echo "WARN: OniAIKit resource bundle not found at $ONIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OniAI"
killall -q OniAI 2>/dev/null || true
