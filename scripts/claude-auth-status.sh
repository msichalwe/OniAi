...
# Checks both Claude Code and OniAI auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ONI_AUTH="$HOME/.oni/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    oni models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    oni_status=$(check_oni_auth 2>/dev/null || true)

    claude_expires=0
    oni_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        oni_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        oni_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ONI_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            oni: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$oni_status" == EXPIRED* ]] || [[ "$oni_status" == MISSING* ]]; then
        echo "ONI_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$oni_status" == EXPIRING* ]]; then
        echo "ONI_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "OniAI Auth (~/.oni/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'oni doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active oni >/dev/null 2>&1; then
    echo -e "OniAI service: ${GREEN}running${NC}"
else
    echo -e "OniAI service: ${RED}NOT running${NC}"
fi
