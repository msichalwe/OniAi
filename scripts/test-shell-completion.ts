 ...
 * `oni update`. Use it to verify the completion installation flow
...
\`oni update\`, \`oni doctor\`, and \`oni onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OniAI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`oni completion --install\` later to enable.`));
    return;
  }
