...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/oni/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OniAI Re-Auth" -c "After getting key, run: ssh $SERVER '~/oni/scripts/mobile-reauth.sh'" --id oni-auth
        ;;
    *)
