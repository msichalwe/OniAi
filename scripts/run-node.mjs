...
const logRunner = (message, deps) => {
  if (deps.env.ONI_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[oni] ${message}\n`);
};

const runOniAI = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["oni.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
