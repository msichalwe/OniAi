...
BASE_IMAGE="${BASE_IMAGE:-oni-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-oni-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=oni.sandbox=1)
NOTE
