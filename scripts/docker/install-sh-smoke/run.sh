...
INSTALL_URL="${ONI_INSTALL_URL:-https://oni.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ONI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ONI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="oni"
PACKAGE_NAME="${ONI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ONI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ONI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ONI_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
