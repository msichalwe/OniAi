#!/usr/bin/env bash
# Reset OniAI like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ONI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="OniAI.app/Contents/MacOS/OniAI"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/OniAI"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/OniAI"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/OniAI"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.oni.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/oni-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ONI_RESTART_LOG:-/tmp/oni-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ONI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.oni/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ONI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node oni.mjs daemon install --force --runtime node"
      log "  node oni.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.oni/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.oni.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing OniAI instances"
kill_all_oni
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product OniAI"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/OniAI.app" ]]; then
    APP_BUNDLE="/Applications/OniAI.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/OniAI.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/OniAI.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/OniAI.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ONI_APP_BUNDLE to your installed OniAI.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node oni.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node oni.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".oni", "oni.json"), "utf8");
        const cfg = JSON.parse(raw);
        const port = cfg && cfg.gateway && typeof cfg.gateway.port === "number" ? cfg.gateway.port : 19100;
        process.stdout.write(String(port));
      } catch {
        process.stdout.write("19100");
      }
    '
  )"
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: OniAI is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.oni.gateway.plist' | head -n 40 || true"
fi
