...
#
# Suggested cron: */30 * * * * /home/admin/oni/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OniAI notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., oni-alerts)
...
STATE_FILE="$HOME/.oni/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OniAI if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use oni
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OniAI to $NOTIFY_PHONE..."
            oni send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OniAI is down. Run: ssh l36 '~/oni/scripts/mobile-reauth.sh'" "urgent"
    exit 1
