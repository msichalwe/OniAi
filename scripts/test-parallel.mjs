...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/oni-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// ONI_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.ONI_TEST_VM_FORKS === "1" ||
  (process.env.ONI_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks && !lowMemLocalHost);
const disableIsolation = process.env.ONI_TEST_NO_ISOLATE === "1";
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.ONI_TEST_SHARDS ?? "", 10);
...
const silentArgs =
  process.env.ONI_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.ONI_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.ONI_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
function resolveReportDir() {
  const raw = process.env.ONI_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
