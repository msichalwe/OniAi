#!/usr/bin/env bash
# ClawDock - Docker helpers for OniAI
# Inspired by Simon Willison's "Running OniAI in Docker"
# https://til.simonwillison.net/llms/oni-docker
#
# Installation:
...
# Common paths to check for OniAI
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/oni"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OniAI not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/oni/oni.git ~/oni"
    echo "  cd ~/oni && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ONI_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d oni-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart oni-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f oni-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec oni-gateway \
    bash -c 'echo "alias oni=\"./oni.mjs\"" > /tmp/.bashrc_oni && bash --rcfile /tmp/.bashrc_oni'
}
...
clawdock-cli() {
  _clawdock_compose run --rm oni-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build oni-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ONI_GATEWAY_TOKEN=$token" oni-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" oni-gateway \
    bash -c './oni.mjs config set gateway.remote.token "$TOKEN" && ./oni.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec oni-gateway \
    bash -c "./oni.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart oni-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec oni-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'oni config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (oni alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to oni project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.oni)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'oni channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'oni status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.oni.ai${_CLR_RESET}"
  echo ""
}
