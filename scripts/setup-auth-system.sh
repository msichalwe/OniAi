#!/bin/bash
# Setup OniAI Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'oni-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OniAI message: Send warning via OniAI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/oni-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/oni-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/oni-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now oni-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status oni-auth-monitor.timer"
echo ""
echo "Quick commands:"
