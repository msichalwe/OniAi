...
- Agents/Project Context: add project scanner (`src/projects/`) that auto-detects tech stack, framework, package manager, test runner, linter, database, scripts, changelog, README, git branch, and directory structure from any workspace. Produces `.oni-project.json` with formatted prompt injection for agent sessions.
- Agents/Coding Agents: add coding agent integration (`src/projects/coding-agents.ts`) with built-in support for Claude Code, GPT Codex, and Aider CLIs. Includes command builders, default configs with 30-min timeouts, background exec patterns for long-running dev tasks, and system prompt guidance for short/medium/long task workflows.
- Memory/Auto-Extract: add automatic memory extraction system (`src/memory/auto-extract.ts`) with structured daily memory files, MEMORY.md section management with deduplication, recent memory context builder, and session bridge context for cross-session continuity.
- Memory/Post-Compaction: enhance post-compaction context injection to include recent memory entries alongside AGENTS.md critical sections, ensuring the agent retains context continuity after compaction.
- Memory/Bubble Tool: add `memory_bubble` agent tool for structured persistent memory — agents can now add/query/delete bubbles, manage entities (people/projects/tools) with deduplication and aliases, build typed relationships with graph traversal, store user preferences with reinforcement, and maintain a dynamic user profile across sessions. Backed by SQLite via `UnifiedMemoryStore`.
- Memory/Unified Store: activate `UnifiedMemoryStore` (SQLite) as the primary bubble backend, replacing the JSON `BubbleStore`. Adds FTS for bubble content, multi-hop graph traversal via recursive CTE, importance decay, history tracking, agent-scoped bubbles, delete operations, proactive surfacing (`surfaceRelevant`/`buildContextPrompt`), and node scan tracking. Auto-migrates from legacy JSON store on first use.
- Memory/Integration: migrate `heartbeat-surfacing.ts`, `node-scanner.ts`, and `post-compaction-context.ts` to prefer `UnifiedMemoryStore` with fallback to legacy JSON `BubbleStore`. Add `memory_bubble` to `group:memory` tool catalog, subagent deny list, system prompt guidance, and sandbox tool policies.
- Memory/Human-Like Extraction: add automatic entity, relationship, and preference extraction from every conversation turn via pattern-based NLP (`auto-entity-extract.ts`). Recognizes family relationships ("my wife Chioni"), friendships, preferences ("call me Mr S", "I prefer dark mode"), and workflow patterns. Entities are deduplicated via `find_entity` before creation.
- Memory/Graph Visualization: add `graph_summary`, `entity_detail`, and `fuzzy_find_entity` actions to `memory_bubble` tool. `graph_summary` shows the full entity + relationship map, `entity_detail` shows deep-dive with linked bubbles and resolved relationship names, `fuzzy_find_entity` matches partial names and fact content.
- Memory/Relationship Types: add family and organizational relationship types to `RelationType`: `spouse`, `parent`, `child`, `sibling`, `manages`, `reports-to`.
- Memory/Proactive Prompting: enhance system prompt to instruct agents to auto-extract entities, relationships, and preferences from every user message silently — always `find_entity` before `add_entity` to avoid duplicates, cross-reference later mentions to existing entities.
- Fix/Gateway Auth: fix `resolveGatewayOptions` to resolve local gateway token from config when no URL override is provided. Previously, agent tools (e.g. `nodes`) called from within the gateway (heartbeat) would connect with no auth token, causing 304 "gateway token mismatch" errors and 15-minute stuck heartbeat loops.
- Fix/Loop Detection: enable tool loop detection by default (was opt-in, allowing unbounded tool retries). Add `nodes` (action=status) to known poll tools. Promote generic-repeat detector from warn-only to critical-block at `criticalThreshold` (20 calls). This prevents agents from retrying the same failing tool call hundreds of times.
- Fix/Heartbeat Timeout: cap heartbeat run timeout at 2 minutes (was inheriting the full 10-minute agent timeout). Prevents stuck heartbeat cycles from blocking for 10–15 minutes.
- Fix/Node Scan Prompt: gate heartbeat node scan prompt to instruct the agent to check node availability first and skip silently on failure, instead of blindly calling `nodes` tool when no nodes may be connected.
- Skills/Always-On Enforcement: `always: true` skills now bypass `enabled: false` config overrides — core skills cannot be accidentally disabled. Added regression tests for `shouldIncludeSkill`.
- Agent/WAL Protocol: add Write-Ahead Logging section to system prompt — agent must persist corrections, proper nouns, preferences, decisions, and specific values via `memory_bubble` BEFORE responding. "Write first, respond second."
- Agent/Compaction Recovery: add Compaction Recovery protocol to system prompt — after context truncation, agent auto-recovers by running `memory_search`, `memory_bubble(query_bubbles)`, `memory_bubble(graph_summary)`, `plan(get)`, and `task(list)` before responding. Never asks "what were we discussing?"
- Agent/Context Danger Zone: add context usage monitoring to system prompt — agent starts persisting key details at ~60% context usage, persists aggressively at ~80%, and follows Compaction Recovery after truncation.
- Agent/Autonomous Initiative: add proactive behavior section to system prompt — anticipate needs, self-heal (try 5-10 approaches before asking), verify before reporting, follow through with task/plan tools, recognize patterns and propose automation.
- Agent/Learning Loop: upgrade Learning Loop to use `memory_bubble` as primary persistence layer with specific action mappings for corrections, errors, discoveries, people, preferences, and task completion.
- Agent/Heartbeat: upgrade default heartbeat prompt from passive "read HEARTBEAT.md" to an autonomous work cycle: task(next) → plan(get) → HEARTBEAT.md → self-check via memory_bubble → HEARTBEAT_OK. Drives proactive agentic behavior on every heartbeat cycle.
- Skills/Proactive Agent: add bundled `proactive-agent` skill — WAL protocol (write-ahead logging for corrections, decisions, preferences before responding), working buffer protocol (persist context as compaction approaches), compaction recovery (auto-recover from context loss via memory_search + memory_bubble), unified search protocol, verify-before-reporting, relentless resourcefulness, reverse prompting, and growth loops (curiosity, pattern recognition, outcome tracking). Always-on.
- Skills/Self-Improving: add bundled `self-improving` skill — automatic detection of corrections, errors, knowledge gaps, and feature requests with structured logging via memory_bubble. Includes recurring pattern detection, priority guidelines, promotion to permanent memory, and periodic review protocol. Always-on.
- Skills/Humanizer: add bundled `humanizer` skill — identifies and removes 24 AI writing patterns based on Wikipedia's "Signs of AI writing" guide. Covers content patterns (inflated significance, promotional language, vague attributions), language patterns (AI vocabulary, copula avoidance, rule of three, synonym cycling), style patterns (em dash overuse, boldface, emoji decoration), communication patterns (sycophantic tone, knowledge-cutoff disclaimers), and filler/hedging. Always-on.
- Skills/Document Editing: add bundled `document-editing` skill — create, read, edit, and convert DOCX, Excel (XLSX), CSV, JSON, XML, HTML, and Markdown documents using Python scripts (python-docx, openpyxl, stdlib csv/json/xml) and pandoc. Includes recipes for find-and-replace in DOCX, Excel with charts and formulas, CSV filtering, JSON manipulation, XML editing, and cross-format conversion. Always-on.
- Gateway/Interactive Mode: add gateway-level `/interactive` command for always-on ambient AI with mic, camera, and screen awareness. Features: hybrid intent detection (wake word + LLM micro-classifier + push-to-talk + follow-up window), per-session sliding-window rate limiting (transcription/vision/classifier), chunked audio transcription pipeline with Whisper hallucination filtering, vision context pipeline (camera + screen frames, throttled, in-memory only), action loop orchestrator with agent integration and optional TTS auto-reply. All processing uses existing gateway-configured providers — zero new API keys or external dependencies. New config section: `interactive` in `oni.json`. Gateway WS methods: `interactive.start/stop/enable/disable/status/configure/audio.chunk/audio.end/frame/ptt`. Slash commands: `/interactive [enable|disable|status] [mic,camera,screen,ambient]`, `/exit interactive`.
- Gateway/Interactive Wiring: fully wire interactive mode end-to-end. Register all `interactive.*` methods in gateway method list and events in broadcast event list. Create ActionLoop with transcribe/classify/sendToAgent callbacks on `interactive.start`. Add WS handlers for audio chunk/end, vision frame, and push-to-talk ingestion. Fix `onStateChange` broadcast registration (register once, not per start call). Classify interactive methods in role/scope authorization. Register `/interactive` as a TUI slash command with argument completions. Wire `interactive.*` broadcast events into TUI display (state transitions, transcripts, responses, actions). Add `request()` passthrough to `GatewayChatClient`.
- Skills/Web Scraping: add bundled `web-scraping` skill — extract structured data from websites using curl, Python stdlib (urllib, html.parser), jq, and beautifulsoup4. Covers HTML text extraction, link extraction, table-to-CSV, REST API pagination, CSS selector parsing, and file downloading. Always-on.
- Skills/Data Analysis: add bundled `data-analysis` skill — analyze datasets with summary stats, group-by aggregation, pivot tables, and time series using Python stdlib or pandas. Includes chart generation (bar, line, pie) via matplotlib, export to Excel/Markdown, and best practices for large datasets. Always-on.
- Skills/File Conversion: add bundled `file-convert` skill — convert between image formats (PNG/JPG/WebP/SVG via sips/ImageMagick), audio (MP3/WAV/FLAC/M4A via ffmpeg), video (MP4/WebM/GIF via ffmpeg), documents (PDF/DOCX/MD/HTML via pandoc), and data formats (CSV/JSON/XML/YAML via Python). Always-on.
- Skills/Automation: add bundled `automation` skill — build automation workflows using Oni's native cron, task, and event tools alongside standard bash/Python patterns. Covers safe script templates, batch processing, parallel execution, file watchers, retry with backoff, webhook listeners, notification pipelines, database backups, log rotation, and git automation. Always-on.
- Memory/Flush: upgrade pre-compaction memory flush prompt to extract structured categories (decisions, preferences, project facts, task outcomes, error fixes, continuation context) instead of generic "store durable memories."
- Apps/Oni Companion: add the `apps/oni` real-time companion foundation with screen capture, camera capture, browser voice input/output, a streaming thinking layer, and the `POST /api/think` vision/voice analysis endpoint documented in `docs/openapi.yaml` with REST client examples under `scripts/http/oni.http`.
