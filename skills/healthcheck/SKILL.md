---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OniAI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OniAI cron scheduling for periodic checks, or version status checks on a machine running OniAI (laptop, workstation, Pi, VPS).
---

# OniAI Host Hardening

## Overview

Assess and harden the host running OniAI, then align it to a user-defined risk tolerance without breaking access. Use OniAI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OniAI changes the host firewall, SSH, or OS updates; it does not.
...
5. OniAI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OniAI can access sensitive data.
...
### 2) Run OniAI security audits (read-only)

As part of the default read-only checks, run `oni security audit --deep`. Only offer alternatives if the user requests them:

1. `oni security audit` (faster, non-probing)
2. `oni security audit --json` (structured output)

Offer to apply OniAI safe defaults (numbered):

1. `oni security audit --fix`

Be explicit that `--fix` only tightens OniAI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OniAI version/update status (read-only)

As part of the default read-only checks, run `oni update status`.
...
- Credential hygiene notes (location of OniAI creds, prefer disk encryption)
...
After OniAI install or first hardening pass, run at least one baseline audit and version check:

- `oni security audit`
- `oni security audit --deep`
- `oni update status`

Ongoing monitoring is recommended. Use the OniAI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `oni cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `oni update status`
...
Before creating, `oni cron list` and match on exact `name`. If found, `oni cron edit <id> ...`.
If not found, `oni cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `oni update status` (preferred for source checkouts and channels)
2. `npm view oni version` (published npm version)

## OniAI command accuracy

Use only supported commands and flags:

- `oni security audit [--deep] [--fix] [--json]`
- `oni status` / `oni status --deep`
- `oni health --json`
- `oni update status`
- `oni cron add|list|runs|run`

Do not invent CLI flags or imply OniAI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OniAI compaction:
