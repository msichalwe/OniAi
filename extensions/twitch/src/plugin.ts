/**
 * Twitch channel plugin for OniAI.
 *
...
import { buildChannelConfigSchema } from "oni/plugin-sdk";
 ...
 * for OniAI. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OniAIConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OniAIConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OniAIConfig): boolean => {
