...
Official Voice Call plugin for **OniAI**.
...
Docs: `https://docs.oni.ai/plugins/voice-call`
Plugin system: `https://docs.oni.ai/plugin`

## Install (local dev)

### Option A: install via OniAI (recommended)
...
```bash
mkdir -p ~/.oni/extensions
cp -R extensions/voice-call ~/.oni/extensions/voice-call
cd ~/.oni/extensions/voice-call && pnpm install
```
...
```bash
oni voicecall call --to "+15555550123" --message "Hello from OniAI"
oni voicecall continue --call-id <id> --message "Any questions?"
oni voicecall speak --call-id <id> --message "One moment"
