...
function resolveOniAIRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ONI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ONI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOniAIRoot(), "dist", "extensionAPI.js");
