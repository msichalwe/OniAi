...
const DEFAULT_GATEWAY_PORT = 19100;
...
function resolveGatewayPort(cfg: OniAIPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ONI_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: OniAIPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OniAIPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OniAI iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OniAI iOS app:",
            "",
            "```",
            qrAscii,
            "```",
