...
vi.mock("oni/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("oni/plugin-sdk")>("oni/plugin-sdk");
  return {
    ...actual,
...
import type { OniAIPluginServiceContext } from "oni/plugin-sdk";
import { emitDiagnosticEvent } from "oni/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/oni-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): OniAIPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("oni.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("oni.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("oni.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("oni.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("oni.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("oni.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("oni.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("oni.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("oni.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("oni.webhook.processed");
    expect(spanNames).toContain("oni.message.processed");
    expect(spanNames).toContain("oni.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["oni.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("oni.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "oni.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["oni.reason"]).toBe("string");
    expect(String(attrs?.["oni.reason"])).not.toContain(
