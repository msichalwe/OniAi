...
import type { OniAIPluginApi, OniAIPluginService } from "oni/plugin-sdk";
...
function normalizeDenyList(cfg: OniAIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OniAIPluginApi["config"]): string[] {
...
export default function register(api: OniAIPluginApi) {
  ...
  const timerService: OniAIPluginService = {
