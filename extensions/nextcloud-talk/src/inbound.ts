...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as OniAIConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OniAIConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OniAIConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OniAIConfig);
