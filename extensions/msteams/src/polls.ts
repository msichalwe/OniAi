...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["oniPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["oni", "pollId"]) ??
    readNestedString(value, ["oni", "poll", "id"]) ??
    readNestedString(value, ["data", "oniPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "oni", "pollId"]);
  if (!pollId) {
    return null;
  }
