...
metadata: { "oni": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OniAI `sessions_spawn`
- **File I/O** == OniAI `read`/`write`
- **Remote fetch** == OniAI `web_fetch` (or `exec` with curl when POST is required)
