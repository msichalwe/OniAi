...
`oni.invoke --each`) without adding new OniAI code per workflow.
...
This extension depends on OniAI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OniAI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.oni/extensions` as a standalone plugin directory.
