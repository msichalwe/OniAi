...
import Ajv from "ajv";
import { resolvePreferredOniAITmpDir } from "oni/plugin-sdk";
// NOTE: This extension is intended to be bundled with OniAI.
// When running from source (tests/dev), OniAI internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { OniAIPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: OniAIPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via oni.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredOniAITmpDir(), "oni-llm-task-"),
        );
