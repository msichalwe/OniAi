...
describe("zalouser send helpers", () => {
  ...
  it("uses env profile fallback and builds link command", async () => {
    ...
    const result = await sendLinkZalouser("thread-5", " https://oni.ai ", { isGroup: true });

    expect(mockRunZca).toHaveBeenCalledWith(
      ["msg", "link", "thread-5", "https://oni.ai", "-g"],
      { profile: "env-profile" },
    );
  ...
  it("returns caught command errors", async () => {
    ...
    await expect(sendLinkZalouser("thread-6", "https://oni.ai")).resolves.toEqual({
      ok: false,
