...
OniAI extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
oni channels login --channel zalouser
...
```bash
oni message send --channel zalouser --target <threadId> --message "Hello from OniAI!"
```
...
```bash
oni channels login --channel zalouser              # Login via QR
oni channels login --channel zalouser --account work
oni channels status --probe
oni channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
oni directory self --channel zalouser
oni directory peers list --channel zalouser --query "name"
oni directory groups list --channel zalouser --query "work"
oni directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
oni message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
oni message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `oni channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
oni channels login --channel zalouser --account work
oni message send --channel zalouser --account work --target <id> --message "Hello"
