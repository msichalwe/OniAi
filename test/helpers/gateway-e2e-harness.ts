...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `oni-e2e-${name}-`));
  const configDir = path.join(homeDir, ".oni");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ONI_SKIP_CHANNELS: "1",
          ONI_SKIP_PROVIDERS: "1",
          ONI_SKIP_GMAIL_WATCHER: "1",
          ONI_SKIP_CRON: "1",
          ONI_SKIP_BROWSER_CONTROL_SERVER: "1",
          ONI_SKIP_CANVAS_HOST: "1",
          ONI_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
