...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ONI_HOME", snapshot.oniHome);
  restoreKey("ONI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ONI_HOME.
  delete process.env.ONI_HOME;
  process.env.ONI_STATE_DIR = path.join(base, ".oni");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "oni-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".oni", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
