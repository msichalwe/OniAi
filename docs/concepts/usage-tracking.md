...
- `/usage cost` in chats: local cost summary aggregated from OniAI session logs.
- CLI: `oni status --usage` prints a full per-provider breakdown.
- CLI: `oni channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
