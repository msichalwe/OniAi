---
summary: "Use Venice AI privacy-focused models in OniAI"
read_when:
  - You want privacy-focused inference in OniAI
...
```bash
oni onboard --auth-choice venice-api-key
```
...
```bash
oni agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, OniAI shows all available Venice models. Pick based on your needs:
...
```bash
oni models set venice/claude-opus-45
oni models set venice/llama-3.3-70b
```

List all available models:

```bash
oni models list | grep venice
```

## Configure via `oni configure`
...
OniAI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
oni agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
oni agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
oni agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
oni agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
oni agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `oni models list` to see currently available models. Some models may be temporarily offline.
