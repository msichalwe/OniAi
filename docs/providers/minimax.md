---
summary: "Use MiniMax M2.1 in OniAI"
read_when:
  - You want MiniMax models in OniAI
...
```bash
oni plugins enable minimax-portal-auth  # skip if already loaded.
oni gateway restart  # restart if gateway is already running
oni onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/oni/oni/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `oni.json`:
...
- Use `oni models list` and `oni models set minimax/MiniMax-M2.1` to switch.
...
- Running `oni configure` and selecting **MiniMax M2.1**, or
