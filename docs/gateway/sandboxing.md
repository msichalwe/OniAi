---
summary: "How OniAI sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
OniAI can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`oni-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; OniAI emits a short-lived token URL that resolves to the observer session.
...
- `"none"` (default): tools see a sandbox workspace under `~/.oni/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
OniAI mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- OniAI blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `oni-sandbox:bookworm-slim`
...
- Use `oni sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
