...
description: "Complete field-by-field reference for ~/.oni/oni.json"
summary: "Complete reference for every OniAI config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.oni/oni.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OniAI uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `oni doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["oni-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-oni",
          requireMention: false,
...
- OniAI additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `oni plugins install @oni/mattermost`.
...
OniAI spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@oni"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `oni.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OniAI auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OniAI assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.oni/sandboxes`
...
noVNC observer access uses VNC auth by default and OniAI emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `oni-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.oni/workspace",
        agentDir: "~/.oni/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:oni`   | All built-in tools (excludes provider plugins)                                           |
...
OniAI uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.oni/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ONI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `oni onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `oni onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `oni onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `oni onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `oni onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `oni onboard --auth-choice minimax-api`.
...
- Loaded from `~/.oni/extensions`, `<workspace>/.oni/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ONI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ONI_GATEWAY_PORT` > `gateway.port` > `19100`.
...
```bash
ONI_CONFIG_PATH=~/.oni/a.json \
ONI_STATE_DIR=~/.oni-a \
oni gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.oni-dev` + port `19001`), `--profile <name>` (uses `~/.oni-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.oni/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-oni-token: <token>`.
...
```json5
{
  hooks: {
    gmail: {
      ...
      hookUrl: "http://127.0.0.1:19100/hooks/gmail",
      includeBody: true,
      maxBytes: 20000,
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ONI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ONI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__oni__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__oni__/a2ui/`
...
- Also serves A2UI at `/__oni__/a2ui/`.
...
- Hostname defaults to `oni`. Override with `ONI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.oni/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `oni dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.oni/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.oni/credentials/oauth.json`.
...
- Default log file: `/tmp/oni/oni-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `oni doctor --fix` can strip unknown keys).
