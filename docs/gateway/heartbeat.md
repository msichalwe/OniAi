...
- During heartbeat runs, OniAI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `oni sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OniAI skips the heartbeat run entirely (no model call).
...
```bash
oni system event --text "Check for urgent follow-ups" --mode now
```
