...
read_when:
  - Running OniAI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OniAI
...
- You run OniAI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OniAI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OniAI extracts the user identity from the configured header
...
- OniAI can stay on loopback HTTP behind the proxy.
...
If OniAI itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://oni.example.com
    to: http://oni-gateway:19100
    policy:
      - allow:
          or:
            - email:
...
`oni security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OniAI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `oni security audit` and review findings
