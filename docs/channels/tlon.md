...
Tlon is a decentralized messenger built on Urbit. OniAI connects to your Urbit ship and can
...
```bash
oni plugins install @oni/tlon
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/tlon
```
...
By default, OniAI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `oni message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OniAI replies in-thread.
