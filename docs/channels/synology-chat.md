---
summary: "Synology Chat webhook setup and OniAI config"
read_when:
  - Setting up Synology Chat with OniAI
...
```bash
oni plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your OniAI gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in OniAI.
...
- Pairing approvals work with:
  - `oni pairing list synology-chat`
  - `oni pairing approve synology-chat <CODE>`
...
```bash
oni message send --channel synology-chat --target 123456 --text "Hello from OniAI"
oni message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
