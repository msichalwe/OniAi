---
summary: "Mattermost bot setup and OniAI config"
read_when:
...
```bash
oni plugins install @oni/mattermost
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/mattermost
```
...
4. Configure OniAI and start the gateway.
...
- Approve via:
  - `oni pairing list mattermost`
  - `oni pairing approve mattermost <CODE>`
...
Use these target formats with `oni message send` or cron/webhooks:
