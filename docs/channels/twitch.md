...
read_when:
  - Setting up Twitch chat integration for OniAI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OniAI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
oni plugins install @oni/twitch
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ONI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "oni", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ONI_TWITCH_ACCESS_TOKEN env var)
...
```bash
ONI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
