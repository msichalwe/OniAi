...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OniAI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
oni plugins install @oni/feishu
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/feishu
```
...
If you just installed OniAI, run the wizard:
...
2. Configuring app credentials in OniAI
...
1. You already ran `oni channels add` for Feishu
2. The gateway is running (`oni gateway status`)
...
```bash
oni pairing approve feishu <CODE>
```
...
2. Run `oni logs --follow` and look for `chat_id`
...
2. Run `oni logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `oni gateway status`  | Show gateway status           |
| `oni gateway install` | Install/start gateway service |
| `oni gateway stop`    | Stop gateway service          |
| `oni gateway restart` | Restart gateway service       |
| `oni logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `oni gateway status`
6. Check logs: `oni logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.oni/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.oni/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
