...
read_when:
  - You want OniAI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OniAI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`oni onboard`) and `oni channels add` list optional channel plugins.
...
```bash
oni plugins install @oni/nostr
```

Use a local checkout (dev workflows):

```bash
oni plugins install --link <path-to-oni>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "oni@example.com",
        "lud16": "oni@example.com"
      }
    }
  }
}
