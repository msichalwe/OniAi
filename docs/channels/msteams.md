...
```bash
oni plugins install @oni/msteams
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OniAI will offer the local install path automatically.
...
3. Configure OniAI with those credentials.
...
- Talk to OniAI via Teams DMs, group chats, or channels.
...
- On startup, OniAI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.oni/oni.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OniAI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `oni-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `oni plugins install @oni/msteams`
   - From a local checkout: `oni plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OniAI in Teams", "full": "OniAI in Teams" },
...
OniAI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OniAI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OniAIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OniAI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `oni message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.oni/msteams-polls.json`.
...
```bash
oni message send --channel msteams \
...
oni message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
oni message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
oni message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
oni message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
