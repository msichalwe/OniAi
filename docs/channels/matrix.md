...
Matrix is an open, decentralized messaging protocol. OniAI connects as a Matrix **user**
...
```bash
oni plugins install @oni/matrix
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `oni plugins install @oni/matrix`
   - From a local checkout: `oni plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OniAI calls the same
     login endpoint, stores the access token in `~/.oni/credentials/matrix/credentials.json`,
...
- On first connection, OniAI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OniAI logs a warning.
...
`~/.oni/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `oni pairing list matrix`
  - `oni pairing approve matrix <CODE>`
...
- On startup, OniAI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
