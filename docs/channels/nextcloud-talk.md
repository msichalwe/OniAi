...
```bash
oni plugins install @oni/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
oni plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OniAI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `oni pairing list nextcloud-talk`
  - `oni pairing approve nextcloud-talk <CODE>`
