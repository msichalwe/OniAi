...
- With Anthropic API-key auth profiles, OniAI seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OniAI injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OniAI exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.oni/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ONI_STATE_DIR/logs/cache-trace.jsonl`
...
- `ONI_CACHE_TRACE=1` enables cache tracing.
- `ONI_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ONI_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ONI_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ONI_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
