...
OniAI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OniAI owns lifecycle when `channels.signal.autoStart=true`.
...
- OniAI spawns `imsg rpc` as a child process (legacy iMessage integration).
