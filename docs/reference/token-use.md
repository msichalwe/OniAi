---
summary: "How OniAI builds prompt context and reports token usage + costs"
read_when:
...
OniAI tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OniAI assembles its own system prompt on every run. It includes:
...
For images, OniAI downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OniAI session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `oni status --usage` and `oni channels list` show
...
`cacheWrite`. If pricing is missing, OniAI shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OniAI can
...
Anthropic's 1M context window is currently beta-gated. OniAI can inject the
...
OniAI skips the `context-1m-*` beta header because Anthropic currently
