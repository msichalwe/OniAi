...
OniAI features that can generate provider usage or paid API calls.
...
- `oni status --usage` and `oni channels list` show provider **usage windows**
...
OniAI can pick up credentials from:
...
`oni models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
