...
read_when:
  - Designing workspace memory (~/.oni/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OniAI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.oni/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.oni/workspace` as canonical human-readable memory.
...
```
~/.oni/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OniAI**, but keep a separable core library.

### Why integrate into OniAI?
...
- You want the agent itself to call the tools:
  - `oni memory recall "…" --k 25 --since 30d`
  - `oni memory reflect --since 7d`
...
Pragmatic take for `~/.oni/workspace`:
