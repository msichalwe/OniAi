...
- Publish as `oni/plugin-sdk` (or export from core under `oni/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OniAIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OniAIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OniAIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OniAIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OniAIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OniAIConfig): string;
  };
};
```
...
- Introduce `oni/plugin-sdk`.
- Add `api.runtime` to `OniAIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `oniRuntime: ">=2026.2.0"`).
