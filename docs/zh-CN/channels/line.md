...
LINE 通过 LINE Messaging API 连接到 OniAI。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
oni plugins install @oni/line
```

本地检出（从 git 仓库运行时）：

```bash
oni plugins install ./extensions/line
```
...
```bash
oni pairing list line
oni pairing approve line <CODE>
```
