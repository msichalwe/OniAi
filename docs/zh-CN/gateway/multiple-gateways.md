...
summary: 在同一主机上运行多个 OniAI Gateway 网关（隔离、端口和配置文件）
title: 多 Gateway 网关
x-i18n:
...
- `ONIAI_CONFIG_PATH` — 每个实例的配置文件
- `ONIAI_STATE_DIR` — 每个实例的会话、凭证、缓存
...
配置文件自动限定 `ONIAI_STATE_DIR` + `ONIAI_CONFIG_PATH` 范围并为服务名称添加后缀。

```bash
# main
oni --profile main setup
oni --profile main gateway --port 18789
...
```bash
ONIAI_CONFIG_PATH=~/.oni/main.json \
ONIAI_STATE_DIR=~/.oni-main \
oni gateway --port 18789

ONIAI_CONFIG_PATH=~/.oni/rescue.json \
ONIAI_STATE_DIR=~/.oni-rescue \
