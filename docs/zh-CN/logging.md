...
所有日志配置都在 `~/.oni/oni.json` 的 `logging` 下。
...
- OniAI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
ONIAI_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `oni plugins enable diagnostics-otel` 启用插件。
...
- `oni.tokens`（计数器，属性：`oni.token`、`oni.channel`、`oni.provider`、`oni.model`）
- `oni.cost.usd`（计数器，属性：`oni.channel`、`oni.provider`、`oni.model`）
- `oni.run.duration_ms`（直方图，属性：`oni.channel`、`oni.provider`、`oni.model`）
- `oni.context.tokens`（直方图，属性：`oni.context`、`oni.channel`、`oni.provider`、`oni.model`）

消息流：

- `oni.webhook.received`（计数器，属性：`oni.channel`、`oni.webhook`）
- `oni.webhook.error`（计数器，属性：`oni.channel`、`oni.webhook`）
- `oni.webhook.duration_ms`（直方图，属性：`oni.channel`、`oni.webhook`）
- `oni.message.queued`（计数器，属性：`oni.channel`、`oni.source`）
- `oni.message.processed`（计数器，属性：`oni.channel`、`oni.outcome`）
- `oni.message.duration_ms`（直方图，属性：`oni.channel`、`oni.outcome`）

队列 + 会话：

- `oni.queue.lane.enqueue`（计数器，属性：`oni.lane`）
- `oni.queue.lane.dequeue`（计数器，属性：`oni.lane`）
- `oni.queue.depth`（直方图，属性：`oni.lane` 或 `oni.channel=heartbeat`）
- `oni.queue.wait_ms`（直方图，属性：`oni.lane`）
- `oni.session.state`（计数器，属性：`oni.state`、`oni.reason`）
- `oni.session.stuck`（计数器，属性：`oni.state`）
- `oni.session.stuck_age_ms`（直方图，属性：`oni.state`）
- `oni.run.attempt`（计数器，属性：`oni.attempt`）
...
- `oni.webhook.error`
  - `oni.channel`、`oni.webhook`、`oni.chatId`、`oni.error`
- `oni.message.processed`
  - `oni.channel`、`oni.outcome`、`oni.chatId`、`oni.messageId`、`oni.sessionKey`、`oni.sessionId`、`oni.reason`
- `oni.session.stuck`
  - `oni.state`、`oni.ageMs`、`oni.queueDepth`、`oni.sessionKey`、`oni.sessionId`
