...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `oni sandbox explain`。
...
旧版 `agent.*` 配置由 `oni doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
...
4. **监控日志：**
   ```exec
   tail -f "${ONIAI_STATE_DIR:-$HOME/.oni}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
