...
- `~/Library/Application Support/OniAI/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `oni-canvas://<session>/<path>`

示例：

- `oni-canvas://main/` → `<canvasRoot>/main/index.html`
- `oni-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `oni-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
oni nodes canvas present --node <id>
oni nodes canvas navigate --node <id> --url "/"
oni nodes canvas eval --node <id> --js "document.title"
oni nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__oni__/a2ui/
```
...
oni nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
oni nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "oni://agent?message=Review%20this%20design";
```
