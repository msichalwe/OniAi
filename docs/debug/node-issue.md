...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OniAI
...
Running OniAI via Node with `tsx` fails at startup with:

```
[oni] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `oni status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch oni.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node oni.mjs status` works on Node 25.
