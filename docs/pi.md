...
summary: "Architecture of OniAI's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in OniAI
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how OniAI integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

OniAI uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, OniAI directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in OniAI's local TUI mode)                                             |
...
├── pi-tools.ts                    # createOniAICodingTools()
...
├── oni-tools.ts              # OniAI-specific tools
...
2. **Custom Replacements**: OniAI replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **OniAI Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures OniAI's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, OniAI CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
OniAI wraps this with `guardSessionManager()` for tool result safety.
...
OniAI maintains an auth profile store with multiple API keys per provider:
...
OniAI loads custom pi extensions for specialized behavior:
...
OniAI also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | OniAI Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom OniAI tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.oni/agents/<agentId>/sessions/` (or `$ONI_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ONI_LIVE_TEST=1`)
