---
title: Lobster
summary: "Typed workflow runtime for OniAI with resumable approval gates."
description: Typed workflow runtime for OniAI — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets OniAI run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: OniAI runs one Lobster tool call and gets a structured result.
...
OniAI launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | oni.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
oni.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In OniAI tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the OniAI Gateway (see the [Lobster repo](https://github.com/oni/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), OniAI keeps core tools enabled. To restrict core
...
→ oni calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ oni calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls OniAI tools that do.
