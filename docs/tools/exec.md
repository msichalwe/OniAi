...
- `gateway`/`node` approvals are controlled by `~/.oni/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  OniAI prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`oni security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `oni doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
