...
- **gateway host** → `oni` process on the gateway machine
...
- allowlist entries live in host-local `~/.oni/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `oni approvals allowlist ...`).
- `oni security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `oni doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.oni/exec-approvals.json` directly.

CLI: `oni approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
