---
summary: "Chrome extension: let OniAI drive your existing Chrome tab"
read_when:
...
The OniAI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate oni-managed Chrome profile.
...
OniAI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OniAI release (npm package) as static files. There is no separate “build” step.

After upgrading OniAI:

- Re-run `oni browser extension install` to refresh the installed files under your OniAI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OniAI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ONI_GATEWAY_TOKEN`)
...
- Open the tab you want OniAI to control.
...
- `ON`: attached; OniAI can drive that tab.
...
`oni browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `oni browser extension install` first to copy the extension to a stable location under your OniAI state directory.
...
- **This is not isolated** like the dedicated oni-managed profile.
