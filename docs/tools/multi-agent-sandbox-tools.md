...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `oni sandbox explain`.
...
Legacy `agent.*` configs are migrated by `oni doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ONI_STATE_DIR:-$HOME/.oni}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
