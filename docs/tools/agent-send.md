---
summary: "Direct `oni agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `oni agent` (direct agent runs)

`oni agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `oni message --target`).
...
```bash
oni agent --to +15555550123 --message "status update"
...
oni agent --to +15555550123 --message "Summon reply" --deliver
