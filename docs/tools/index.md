---
summary: "Agent tool surface for OniAI (browser, canvas, nodes, message, cron) replacing legacy `oni-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `oni-*` skills
title: "Tools"
---

# Tools (OniAI)

OniAI exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `oni-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `oni.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OniAI logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:oni`: all built-in OniAI tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`oni node run`).
...
OniAI tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `oni configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OniAI-managed browser.
...
- Quick smoke: `oni nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `oni gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, OniAI uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OniAI clamps `tools.sessions.visibility` to `tree`.
...
- `gatewayUrl` (default `ws://127.0.0.1:19100`)
