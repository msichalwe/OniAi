...
read_when:
  - You need detailed behavior for oni onboard
...
This page is the full reference for `oni onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.oni/oni.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `oni doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `oni pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `oni health`.
    - `oni status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.oni/.env` so launchd can read it.
...
- OAuth credentials: `~/.oni/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.oni/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.oni/credentials/oauth.json` (or `$ONI_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`oni onboard`](/cli/onboard)
