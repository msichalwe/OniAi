...
- default: `http://<host>:19100/`
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/oni`)
...
- [http://127.0.0.1:19100/](http://127.0.0.1:19100/) (or [http://localhost:19100/](http://localhost:19100/))

If the page fails to load, start the Gateway first: `oni gateway`.
...
you revoke it with `oni devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.oni/oni.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop oni`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OniAI
...
```bash
oni gateway --bind tailnet --token "$(openssl rand -hex 32)"
```

Then open:

- `http://<tailscale-ip>:19100/` (or your configured `gateway.controlUi.basePath`)
...
OniAI **blocks** Control UI connections without device identity.

**Recommended fix:** use HTTPS (Tailscale Serve) or open the UI locally:

- `https://<magicdns>/` (Serve)
- `http://127.0.0.1:19100/` (on the gateway host)
...
```bash
ONI_CONTROL_UI_BASE_PATH=/oni/ pnpm ui:build
```
...
Then point the UI at your Gateway WS URL (e.g. `ws://127.0.0.1:19100`).
...
```text
http://localhost:5173/?gatewayUrl=ws://<gateway-host>:19100
```

Optional one-time auth (if needed):

```text
http://localhost:5173/?gatewayUrl=wss://<gateway-host>:19100&token=<gateway-token>
```
