...
```bash
oni tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Check the Gateway logs: `oni logs --follow`.
- Confirm the agent can run: `oni status` and `oni models status`.
...
- No agents in picker: check `oni agents list` and your routing config.
