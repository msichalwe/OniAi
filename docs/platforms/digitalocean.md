---
summary: "OniAI on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OniAI on DigitalOcean
  - Looking for cheap VPS hosting for OniAI
title: "DigitalOcean"
---

# OniAI on DigitalOcean

## Goal

Run a persistent OniAI Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status oni-gateway.service

# View logs
journalctl --user -u oni-gateway.service -f
```
...
ssh -L 19100:localhost:19100 root@YOUR_DROPLET_IP
...
```bash
oni config set gateway.bind tailnet
oni gateway restart
```

Open: `http://<tailscale-ip>:19100` (token required).
...
- `~/.oni/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u oni --no-pager -n 50
```
