---
summary: "OniAI on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OniAI on a Raspberry Pi
  - Running OniAI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OniAI on Raspberry Pi

## Goal

Run a persistent, always-on OniAI Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
ssh -L 19100:localhost:19100 user@gateway-host
...
Most OniAI features work on ARM64, but some external binaries may need ARM builds:
...
```bash
# Check logs
journalctl -u oni --no-pager -n 100

# Common fix: rebuild
cd ~/oni  # if using hackable install
npm run build
sudo systemctl restart oni
```
