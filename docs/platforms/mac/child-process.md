...
service via the external `oni` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.oni.gateway`
  (or `ai.oni.<profile>` when using `--profile`/`ONI_PROFILE`; legacy `com.oni.*` is supported).
