...
- Same path: run the app from a fixed location (for OniAI, `dist/OniAI.app`).
...
```bash
sudo tccutil reset Accessibility ai.oni.mac
sudo tccutil reset ScreenCapture ai.oni.mac
...
Workaround: move files into the OniAI workspace (`~/.oni/workspace`) if you want to avoid per-folder grants.
