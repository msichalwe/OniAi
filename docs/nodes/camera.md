...
OniAI supports **camera capture** for agent workflows:
...
```bash
oni nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
oni nodes camera snap --node <id> --facing front
oni nodes camera clip --node <id> --duration 3000
oni nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`oni.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `oni` CLI to invoke camera commands on the macOS node.

Examples:

```bash
oni nodes camera list --node <id>            # list camera ids
oni nodes camera snap --node <id>            # prints MEDIA:<path>
oni nodes camera snap --node <id> --max-width 1280
oni nodes camera snap --node <id> --delay-ms 2000
oni nodes camera snap --node <id> --device-id <id>
oni nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
oni nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
oni nodes camera clip --node <id> --device-id <id>
oni nodes camera clip --node <id> --no-audio
```

Notes:

- `oni nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
oni nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
