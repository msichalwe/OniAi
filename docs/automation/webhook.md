...
read_when:
  ...
  - Wiring external systems into OniAI
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OniAI config directory (typically `~/.oni/hooks/transforms`).
...
- `oni webhooks gmail setup` writes `hooks.gmail` config for `oni webhooks gmail run`.
...
```bash
curl -X POST http://127.0.0.1:19100/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:19100/hooks/agent \
  -H 'x-oni-token: SECRET' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST http://127.0.0.1:19100/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
