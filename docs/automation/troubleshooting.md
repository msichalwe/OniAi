...
oni system heartbeat last
oni logs --follow
oni config get agents.defaults.heartbeat
oni channels status --probe
```
...
```bash
oni config get agents.defaults.heartbeat.activeHours
oni config get agents.defaults.heartbeat.activeHours.timezone
oni config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
oni cron list
oni logs --follow
```
