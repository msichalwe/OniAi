...
- Jobs persist under `~/.oni/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.oni/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `oni cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OniAI applies a
...
If `delivery` is omitted for isolated jobs, OniAI defaults to `announce`.
...
- Job store: `~/.oni/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.oni/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, OniAI archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `oni cron runs` and adjust retention before logs become large
...
oni cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
oni cron edit <jobId> --agent ops
oni cron edit <jobId> --clear-agent
```
...
```bash
oni system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`oni system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ONI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OniAI applies exponential retry backoff for recurring jobs after consecutive errors:
