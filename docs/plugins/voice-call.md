...
read_when:
  - You want to place an outbound voice call from OniAI
...
Voice calls for OniAI via a plugin. Supports outbound notifications and
...
- Use `oni voicecall ...` or the `voice_call` tool
...
```bash
oni plugins install ./extensions/voice-call
