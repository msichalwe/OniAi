...
This document describes how the OniAI ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`oni acp` exposes an ACP agent over stdio and forwards prompts to a running
OniAI Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `oni acp` and speaks ACP messages over stdio.
...
`oni acp` resolves the Gateway URL and auth from CLI flags or config:
